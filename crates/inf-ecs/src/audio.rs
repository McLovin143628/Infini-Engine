...
mod tests {
    ...
    #[test]
    fn the_engine_names_seventy_six_clips_no_component_ever_references() {
        let clips = active_test_clips();
        assert_eq!(
            clips.len(),
            76,
            "thirty-six report clips, the casing, the venue loop, wave WPN2d's blast plus its two melee surfaces, and the thirty-six vehicle clips (VEH3e's thirty-one, the rolling road's three from its audit among them, and wave VEH3g's five craft voices)"
        );
