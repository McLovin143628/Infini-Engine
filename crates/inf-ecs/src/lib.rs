...
// functions plus one runtime component; the layers that need a world (the
// hold point, the look integrator) are `inf_physics::d3`'s.
pub mod feel;
...
pub use anim_bridge::{
    ...
    clear_anim_bridge, consume_anim_notify, feet_of, footstep_cues, ragdoll_pose, ragdoll_rig,
    request_ragdoll_rig, set_anim_param, set_anim_trigger, set_foot_ik, set_pose_match_entry,
    set_ragdoll_pose, start_throw, take_ragdoll_rig, traversal_arc, AnimBridgeRes, AnimStateInfo,
    FootGoal, FootState, FootstepCue, RagdollBonePose, RagdollPose, RigBone, TraversalArc,
    FOOTSTEP_PREFIX, TRAVERSAL_ARC_SAMPLES,
};
