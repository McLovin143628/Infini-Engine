...
impl ProjectTemplate {
    ...
    /// A committed starter `.inf_lvl` scene scaffolded into the project's Levels
    /// dir, if this template ships one. The Hybrid 2.5D template ships the 2.5D
    /// showcase (a 3D plane + billboarded sprites + a 2D light), and First Person
    /// ships a ground + `CharacterController3D` player + eye-height camera + sun
    /// (both generated by `inf_editor_core::samples`).
    pub fn starter_level(self) -> Option<&'static [u8]> {
        match self {
            ProjectTemplate::Hybrid25d => Some(include_bytes!(
                "../../../templates/hybrid-2.5d/Hybrid.inf_lvl"
            )),
            ProjectTemplate::FirstPerson => Some(include_bytes!(
...
pub fn scaffold(parent: &Path, name: &str, template: ProjectTemplate) -> Result<Scaffolded> {
    ...
    // Templates that ship a starter scene drop it into the Levels dir.
    if let Some(level) = template.starter_level() {
        let path = root.join(&manifest.levels_dir).join("Main.inf_lvl");
...
mod tests {
    ...
    #[test]
    fn hybrid_template_scaffolds_a_starter_level() {
        // `inf new --template hybrid-2.5d` scaffolds a project whose Levels dir
        // contains the committed 2.5D starter scene.
        let parent = tempfile::tempdir().unwrap();
        let t = ProjectTemplate::from_slug("hybrid-2.5d").expect("hybrid slug");
        assert_eq!(t, ProjectTemplate::Hybrid25d);
        let s = scaffold(parent.path(), "My 2.5D", t).unwrap();
        let level = s.root.join(&s.manifest.levels_dir).join("Main.inf_lvl");
        assert!(level.exists(), "hybrid template writes a starter level");
        // It is a real scene payload (non-empty, schema-version-prefixed).
        let bytes = std::fs::read(&level).unwrap();
        assert!(!bytes.is_empty());
        assert_eq!(bytes, ProjectTemplate::Hybrid25d.starter_level().unwrap());
        // The other templates ship no starter level.
        assert!(ProjectTemplate::Blank3d.starter_level().is_none());
    }
