...
/// * **v24** — Wave G, and the wave's **only** scene bump. It carries two
///   unrelated things on purpose, because the alternative is two bumps:
///
///   **(a) The geo-anchor** ([`inf_math::geo::GeoAnchor`]) — where on Earth
///   world `(0, 0, 0)` is, and in what CRS the world plane lies. Without it a
///   DEM, a road centreline and a coastline each arrive in their own numbers and
...
///   read every frame — gravity, tick rate, post-processing, streaming. The
///   anchor is read three times in a level's life (at an import door, by the
///   cook advisory, and to derive the sky's latitude) and is document
///   provenance, sitting naturally beside `title`.
///
///   **(b) `TerrainLayer::material`** — Wave T's named remainder, a real
...
///   sixty-two at v27, and a hundred since v28 grew them again — exactly
...
///      `RenderSettingsRecordV25` and the settings as `RuntimeSettingsV25`; every
...
///      (the `RuntimeEntityGen` idiom, third use) rather than a third
...
///   every other entity.
/// * **v28** — the VEH3 arc's **one** schema window, spent in wave VEH3a after a
///   published price the orchestrator ruled on row by row. [`VehicleClass`]
///   grows from **sixty-two** `f64`s to **a hundred**, and it is again the only
///   component this arc moves — eight waves, one rung.
///
///   The v27 rung is repeated exactly: the pre-v28 component freezes as
///   `VehicleClassV27`, the pre-v28 entity record as [`EntityRecordV27`] and the
///   pre-v28 file as [`SceneFileV27`]; **no re-declared record was needed**, for
///   the third time. The thirty-eight append at the component's **tail**, sorted
///   among themselves, so every v25 and v27 field keeps the offset it has.
///
///   **And a THIRTY-NINTH row, on a second component** — [`SkyAtmosphere`]'s
///   weather block grows `weather_ambient_c`, the **air temperature in °C**
///   (wave VEH3a's audit, inside the same window under the one-window law: a
///   row a wave discovers while the window is open lands in it, or it waits for
///   a window nobody has budgeted). Before it the tyre model derived the air
///   from `weather_snowiness` — the precipitation's *phase* — so "rain falls at
///   20 °C" was a rule of this engine, a cold dry morning was inexpressible, and
///   an author had no control over the number at all. The pre-v28 sky freezes as
///   `SkyAtmosphereV27` (47 fields), which every frozen entity record from
///   [`EntityRecordV14`] forward now carries; the live record's sky slot becomes
///   the FOURTH generic parameter of [`RuntimeEntityGen`] — the `terrain`,
///   `material` and `vehicle_class` slots' own idiom, fourth use — so no record
///   was re-declared for it either. The wire pin gains an independently declared
///   48-field `SkyAtmosphereWire`: **before this the pin named the LIVE
///   `SkyAtmosphere`**, so no version of it could ever have caught a field
///   appended to the sky without a bump.
///
///   The air costs **4 bytes on every entity that carries a sky** — one per
///   level, and **seven of the twenty-four committed levels carry one**, so the
///   whole cost of the row to committed content is **28 bytes** and the other
///   seventeen levels do not grow at all. A pre-v28 sky lifts to the `Clear`
///   preset's **20 °C**, which is exactly the number every pre-v28 tyre already
///   cooled toward, so no committed trace moves.
///
///   What they buy, wave by wave: a **Pacejka magic formula per axis**
///   (`pacejka_lat_b/c/e`, `pacejka_long_b/c/e`) with a `0` sentinel that
///   derives each shipped row's curve from the two knobs it *did* author, so the
///   eleven catalogue rows keep their measured feel; **tyre heat**
///   (`tyre_heat_rate`, `tyre_cool_rate`, `tyre_heat_grip_loss`,
///   `tyre_optimum_c`); `camber_deg`, which is how `WheelContact::normal`
///   finally reaches a force; `relaxation_m`; `tyre_substeps`, the inner-loop
///   count; `tyre_surface_set`, the compound's row of the µ table; a **flywheel
///   and a clutch** (`flywheel_inertia_kgm2`, `clutch_torque_nm`,
///   `clutch_engage_s`); `fuel_cut_rpm`; a **turbo** (`turbo_boost_max`,
///   `turbo_spool_s`, `turbo_lag_s`); **two limited-slip differentials**
///   (`lsd_preload_front_nm`/`_rear_nm`, `lsd_power_ramp_front`/`_rear`,
///   `lsd_coast_ramp_front`/`_rear`); an **engine voice** (`cylinders`,
///   `firing_order_variant`, `engine_voice_kind`); **three damage thresholds**
///   (`glass_health_j`, `panel_health_j`, `part_break_impulse_ns`); and the
///   **air and sea** discriminators (`wing_area_m2`, `wing_aspect_ratio`,
///   `stall_deg`, `planing_speed_mps`, `sail_area_m2`).
///
///   **What was priced and NOT taken**, by name, because a window that carries
///   less than it was budgeted for should say why: the eight boarding sockets
///   (derived at rig time from the def's half-extents and the family part table,
///   which also closes the roof-seat defect for every committed car at once);
///   `door_count` and `hood_hinge` (the family's own part table already is
///   them); the per-surface µ table and every tyre temperature (runtime
///   resources — a surface is a property of the world and a temperature is
///   state, not authored data); a `.inf_vehicle` asset kind and the entity
///   `Uuid` that would name one (the catalogue reaches all three boot paths as
///   a Blueprint class, which is what WPN2's eighty-five-row registry already
///   does); and the per-level `CameraRig` refused by CHAR1c, whose real gap is a
///   write half for the level-side `camera.toml` and costs no schema at all.
///
///   The wire cost is **304 bytes per entity that carries a class** (38 × 8) and
///   **zero** for every other entity — no new record slot, so every non-vehicle
///   entity in all twenty-four committed levels is byte-unchanged but for the
///   schema byte. Twenty-nine entities across those levels carry a class
///   (twenty-six on the island, three in the island fixture), so the whole cost
///   to committed content is **8 816 bytes** for the class and **28 more** for
///   the air, **8 844** in all.
...
pub struct RenderSettingsRecord {
    ...
    //
    // Every field below is appended at the record's tail, and every one is inert
    // at its default. Some have their consumer in this slice (the SSR block) and
    // some in the next (exposure, flare, the lens trio, the Karis downsample) --
    // they land together because the arc gets ONE schema bump, and an unread
    // field is cheap where an unread *wire* field is not: each one is applied by
    // both hosts' `apply_record` and asserted by a store/load arm from the day it
    // lands.
    /// Screen-space reflections. Off means every lit shader's SSR branch is untaken.
    pub ssr_enabled: bool,
    /// Reflection march length, metres.
    pub ssr_distance: f32,
    /// Relative depth-thickness tolerance for accepting a hit.
    pub ssr_thickness: f32,
    /// Step budget: `0` Low (16), `1` Medium (32), `2` High (48). An unknown code
    /// reads as Medium -- `inf_render::SsrQuality::from_code`.
    pub ssr_quality: u8,
    ...
    /// Exposure mode: `0` manual (the multiplier is [`Self::exposure`]), `1`
    /// auto. An unknown code reads as manual -- `inf_render::ExposureMode::from_code`.
    pub exposure_mode: u8,
    /// Exposure compensation, stops.
    pub exposure_compensation_ev: f32,
    /// The dimmest average scene luminance auto exposure adapts to.
    ...
    /// Sun glare / lens flare master enable.
...
impl Default for RenderSettingsRecord {
    fn default() -> Self {
        Self {
            ...
            gi_enabled: false,
            gi_intensity: 1.0,
            // v26. Every value here equals `inf_render`'s own default for the
            // same knob, which is what `apply_record(&default()) ==
            // RenderSettings::default()` pins on both hosts.
            ssr_enabled: false,
            ssr_distance: 24.0,
            ssr_thickness: 0.15,
            ssr_quality: 1,
...
/// The **pre-v26** render block (schema v26 froze this when wave VIS1a appended
/// the photoreal arc's authorable surface). Frozen file records **v8..v25** carry
/// their settings as [`RuntimeSettingsV25`], whose `render` is this shape;
/// [`RenderSettingsRecordV25::into_current`] lifts it with every v26 field at its
/// default.
///
/// The freeze is the standing bincode rule: the block sits *inside*
/// [`RuntimeSettings`], with `partition` after it, so growing it moves every byte
/// of the file's settings that follows.
#[derive(Debug, Clone, Copy, PartialEq, Serialize, Deserialize)]
struct RenderSettingsRecordV25 {
    exposure: f32,
    dither: bool,
    bloom_enabled: bool,
    bloom_threshold: f32,
    ...
    shadows_max_distance: f32,
    gi_enabled: bool,
    gi_intensity: f32,
}

impl RenderSettingsRecordV25 {
    /// Lift to the live block with every v26 field at its default -- which is
    /// exactly what a pre-v26 level meant: no reflections, manual exposure, no
    /// flare, no lens trio.
    fn into_current(self) -> RenderSettingsRecord {
    ...
    /// Project the live block onto the pre-v26 shape (the **downgrade-bless**
    /// path). Everything v26 added is lost, which is precisely what it added.
    #[cfg_attr(not(test), allow(dead_code))]
    fn from_current(r: RenderSettingsRecord) -> Self {
        Self {
            exposure: r.exposure,
            dither: r.dither,
...
/// The **pre-v26** file-settings layout: [`RuntimeSettings`] with the frozen
/// render block. Frozen file records v8..v25 carry this.
#[derive(Debug, Clone, Copy, PartialEq, Serialize, Deserialize)]
struct RuntimeSettingsV25 {
    #[serde(default)]
    gravity_2d: Vec2d,
    #[serde(default = "default_gravity_3d")]
    gravity_3d: Vec3d,
...
impl Default for RuntimeSettingsV25 {
    fn default() -> Self {
        Self::from_current(RuntimeSettings::default())
    }
}
...
///
/// This is the **current** wire layout: field order and the `#[serde(default)]`
/// markers mirror the editor's `EntityRecord` byte-for-byte so the same bincode
/// payload decodes here. Component slots are `Option`s (a slot is `Some` when
/// the entity carries that component).
///
...
> {
    ...
    /// PBR material parameter block. Generic since wave VIS1a / v26 — see the
    /// type's docs for why a third parameter is cheaper than a third restatement.
    ...
    ///
    /// The one generic slot — see the type's own docs.
    #[serde(default)]
...
/// VEH3a). The material and the terrain are the live ones — v28 moved neither.
/// **No new generic parameter was needed**: the tail slot has been generic since
/// v25 for exactly this, and this is its third use.
type EntityRecordV27 =
    RuntimeEntityGen<Terrain, Option<VehicleClassV27>, Material, Option<SkyAtmosphereV27>>;
...
type EntityRecordV26 =
    RuntimeEntityGen<Terrain, Option<VehicleClassV26>, Material, Option<SkyAtmosphereV27>>;
...
type EntityRecordV25 =
    RuntimeEntityGen<Terrain, Option<VehicleClassV26>, MaterialV25, Option<SkyAtmosphereV27>>;
...
type EntityRecordV24 = RuntimeEntityGen<Terrain, (), MaterialV25, Option<SkyAtmosphereV27>>;
...
type EntityRecordV23 = RuntimeEntityGen<TerrainV23, (), MaterialV25, Option<SkyAtmosphereV27>>;

impl EntityRecordV27 {
    /// Lift a frozen v27 record to the live [`RuntimeEntity`]: the thirty-eight
    /// VEH3a tunables arrive at the Ring-0 defaults, which is exactly what every
    /// pre-v28 level meant.
...
impl EntityRecordV24 {
    ...
    /// vehicle tuned by the Ring-0 defaults in both hosts; and the intensity
    /// arrives at 1.0 through the v25 rung.
    fn into_runtime(self) -> RuntimeEntity {
        self.map_tail(|()| None).into_runtime()
    }

    /// Project a live record onto the frozen v24 shape (the **downgrade-bless**
    /// path). The authored class and the intensity are lost.
    #[cfg_attr(not(test), allow(dead_code))]
    fn from_current(r: RuntimeEntity) -> Self {
...
impl<T, V, M, S> RuntimeEntityGen<T, V, M, S> {
    /// Replace the tail slot, moving every other field **wholesale**.
    fn map_tail<W>(self, f: impl FnOnce(V) -> W) -> RuntimeEntityGen<T, W, M, S> {
        self.map_slots(|t| t, f, |m| m, |s| s)
    }

    /// Replace the **terrain** slot, moving every other field wholesale.
    fn map_terrain<U>(self, f: impl FnOnce(T) -> U) -> RuntimeEntityGen<U, V, M, S> {
        self.map_slots(f, |v| v, |m| m, |s| s)
    }

    /// Replace the **material** slot, moving every other field wholesale (wave
    /// VIS1a).
    fn map_material<N>(self, f: impl FnOnce(M) -> N) -> RuntimeEntityGen<T, V, N, S> {
        self.map_slots(|t| t, |v| v, f, |s| s)
    }

    /// Replace the **sky** slot, moving every other field wholesale (schema
    /// v28, wave VEH3a's audit — the `SkyAtmosphere` weather block grew an air
    /// temperature, so every frozen record carries the frozen sky).
    fn map_sky<P>(self, f: impl FnOnce(S) -> P) -> RuntimeEntityGen<T, V, M, P> {
        self.map_slots(|t| t, |v| v, |m| m, f)
    }

    /// Replace all four generic slots, moving every other field **wholesale**.
    ///
    /// The whole point of the parameters: a restatement is where a slot goes
    ...
    fn map_slots<U, W, N, P>(
        self,
        ft: impl FnOnce(T) -> U,
        fv: impl FnOnce(V) -> W,
        fm: impl FnOnce(M) -> N,
        fs: impl FnOnce(S) -> P,
    ) -> RuntimeEntityGen<U, W, N, P> {
...
pub struct RuntimeLevel {
    ...
    ///
    /// Disabled by default, which is what every level authored before v24 meant:
    /// an ordinary made-up world with no place on Earth.
...
/// held between island wave VEH2a and the VEH3 arc's own window.
///
/// The FIFTH freeze of a component, on the standing reason [`MaterialV25`]
/// states and [`VehicleClassV26`] states again: bincode is positional, so
/// *growing* a component is a wire-format change even where every new field is
/// `#[serde(default)]`, and a v27 payload fed to the grown struct reads past the
/// end of its `VehicleClass` and into whatever the record holds next. Declared
/// field-by-field rather than derived from the live one, because a frozen
/// record's whole job is to keep saying what it said when the bytes were
/// written — the fifteen of v25 in their v25 order, then the forty-seven VEH2a
/// appended, and nothing after them.

#[derive(Clone, Copy, Serialize, Deserialize)]
struct VehicleClassV27 {
    brake_force_n: f64,
...
impl VehicleClassV27 {
    /// Lift to the live [`VehicleClass`]: the sixty-two authored numbers are kept
    /// exactly and the thirty-eight VEH3a tunables arrive at the **Ring-0
    /// defaults**, which is what a pre-v28 level meant by not carrying them —
    /// an undriven magic formula (the `0` sentinels derive each row's curve from
    /// the two knobs it did author), no heat model in the trace until a clause
    /// reads one, no turbo, an open differential, a petrol voice, and no wing.
    ///
    /// MIRROR: `inf_editor_core::scene::serialize::VehicleClassV27::into_current`.
...
/// The schema-v28 file layout (current). `entities` reuses [`RuntimeEntity`].
///
...
/// A frozen schema-v27 file layout. v28 grew [`VehicleClass`] (the VEH3 arc's
/// window, wave VEH3a), a change that lives entirely inside the **entity**
/// record, so the file record's own shape and its settings are unchanged and
/// only `entities` is repointed at the frozen [`EntityRecordV27`].
#[derive(Serialize, Deserialize)]
struct SceneFileV27 {
    schema_version: u32,
    title: String,
    entities: Vec<EntityRecordV27>,
    #[serde(default)]
    settings: RuntimeSettings,
    #[serde(default)]
    geo: inf_math::geo::GeoAnchor,
...
/// A frozen schema-v25 file layout. v26 grew `Material` (the emissive intensity)
/// and the render block (the photoreal arc's authorable surface), so BOTH of this
/// record's payload-bearing fields are repointed at frozen shapes -- the entity
/// record at [`EntityRecordV25`] and the settings at [`RuntimeSettingsV25`].
#[derive(Serialize, Deserialize)]
struct SceneFileV25 {
    schema_version: u32,
    title: String,
    entities: Vec<EntityRecordV25>,
    #[serde(default)]
    settings: RuntimeSettingsV25,
    #[serde(default)]
    geo: inf_math::geo::GeoAnchor,
}
...
/// A frozen schema-v23 file layout. v24 appended the geo-anchor to the file
/// record and repointed `entities` at the frozen [`EntityRecordV23`] — the shape
/// this file held as [`RuntimeEntity`] until v24 gave `TerrainLayer` its
/// material binding.
#[derive(Serialize, Deserialize)]
struct SceneFileV23 {
    schema_version: u32,
    title: String,
    entities: Vec<EntityRecordV23>,
    #[serde(default)]
    settings: RuntimeSettingsV25,
}
...
/// gave `Material` its `emissive_intensity`). Frozen entity records **v22..v25**
/// carry `material` as `Option<MaterialV25>`; [`MaterialV25::into_current`] lifts
/// it with an intensity of **1.0**, which is what every pre-v26 level meant: the
/// authored colour, unscaled.
///
/// The THIRD freeze of one component, on the standing reason — bincode is
/// positional, so *growing* a component is a wire-format change even though the
/// new field is `#[serde(default)]`, and a v25 payload fed to the grown struct
/// reads past the end of its `Material` and into the next slot of the same
/// record. `MaterialV7` and `MaterialV21` above stay exactly as they were; the
/// three describe three byte layouts and none is a step toward another.
#[derive(Clone, Copy, Serialize, Deserialize)]
struct MaterialV25 {
    base_color: Color,
    #[serde(default)]
...
impl MaterialV25 {
    ...
    /// **downgrade-bless** path). Only the intensity is lost, which is precisely
    /// what v26 added.
    #[cfg_attr(not(test), allow(dead_code))]
    fn from_current(m: Material) -> Self {
        Self {
...
/// The pre-v26 shape of `Material::default()`, derived from the live default
/// rather than restated — [`MaterialV21`]'s rule, for its reason.
#[cfg(test)]
...
struct RuntimeSettingsV9 {
    ...
    #[serde(default = "default_sim_hz")]
    sim_hz: f64,
    /// Frozen at the pre-v26 shape (wave VIS1a): this record is a *pre-v10* one,
    /// so its render block cannot be the live one either — see
    /// [`RenderSettingsRecordV25`].
    #[serde(default)]
    render: RenderSettingsRecordV25,
}
...
/// The **pre-v24** [`TerrainLayer`] byte layout (schema v24 froze this when
/// Wave G gave every splat layer a real material binding).
///
/// Three fields, and the fourth is exactly what v24 added. A v23 payload's bytes
/// end at `tex_scale`, so decoding one through the grown layer would read the
/// next layer's albedo as this layer's material discriminant — four times per
/// terrain, and then run off the end of the component.
#[derive(Clone, Copy, Default, Serialize, Deserialize)]
struct TerrainLayerV23 {
    albedo: Color,
    #[serde(default)]
    roughness: f64,
    #[serde(default)]
...
impl TerrainLayerV23 {
    /// Lift to the live layer: **no material binding**, which is what a pre-v24
    /// level meant — a solid albedo plus the procedural triplanar grain.
    fn into_current(self) -> TerrainLayer {
        TerrainLayer {
            albedo: self.albedo,
            roughness: self.roughness,
    ...
    /// Project a live layer back (the downgrade-bless path). The material
    /// binding has no v23 home and is dropped — lossy, and used only to
    /// regenerate old fixtures from a current sample.
    #[cfg_attr(not(test), allow(dead_code))]
    fn from_current(l: TerrainLayer) -> Self {
        Self {
            albedo: l.albedo,
    ...
    fn lift_all(
        layers: [TerrainLayerV23; inf_ecs::components::TERRAIN_LAYERS],
    ) -> [TerrainLayer; inf_ecs::components::TERRAIN_LAYERS] {
...
/// The **pre-v24** `Terrain` byte layout. Identical to the live component except
/// that its `layers` carry the frozen [`TerrainLayerV23`]. Frozen entity records
/// v16..v23 carry `terrain` as `Option<TerrainV23>`.
...
struct TerrainV23 {
    ...
    /// Derived, #[serde(skip)] exactly as the live component has it, so the
    /// frozen record and the live one agree byte for byte.
    #[serde(skip)]
    biome_population: Vec<inf_ecs::components::ScatteredInstance>,
}
...
impl std::fmt::Debug for TerrainLayerV23 {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("TerrainLayerV23")
            .field("albedo", &self.albedo)
            .field("roughness", &self.roughness)
            .field("tex_scale", &self.tex_scale)
            .finish()
    }
}

impl PartialEq for TerrainLayerV23 {
    fn eq(&self, other: &Self) -> bool {
        self.albedo == other.albedo
            && self.roughness == other.roughness
            && self.tex_scale == other.tex_scale
    }
}
...
/// audit, inside the one window the VEH3 arc spends). Every frozen entity record
/// from [`EntityRecordV14`] forward carries `sky_atmosphere` as
/// `Option<SkyAtmosphereV27>`, exactly as [`EntityRecordV13`] carries it as
/// `Option<SkyAtmosphereV13>`; [`SkyAtmosphereV27::into_current`] lifts it.
///
/// The 47 fields mirror the v27 component one-for-one **including their
/// `#[serde(default = "...")]` markers**. The pre-v14 half reuses the `v13_*`
/// default fns already frozen beside [`SkyAtmosphereV13`] — the same numbers,
/// already pinned, and a third transcription of them is a third place for a
/// digit to be wrong. The weather half gets its own `v27_*` fns for the same
/// reason the v13 ones exist: a frozen record must not move when the live
/// component's defaults are re-tuned.
#[derive(Clone, Copy, Serialize, Deserialize)]
struct SkyAtmosphereV27 {
    #[serde(default = "v13_sky_true")]
    enabled: bool,
    #[serde(default = "v13_sun_intensity")]
    sun_intensity: f32,
...
impl SkyAtmosphereV27 {
    /// Lift to the live [`SkyAtmosphere`]: the 47 v27 fields carry through
    /// verbatim and `weather_ambient_c` takes its live default — the `Clear`
    /// preset's **20 °C**, which is exactly the number every pre-v28 level's
    /// tyre already cooled toward, so no committed trace moves.
    fn into_current(self) -> SkyAtmosphere {
        SkyAtmosphere {
            enabled: self.enabled,
            sun_intensity: self.sun_intensity,
    ...
    /// Project a live [`SkyAtmosphere`] back onto the frozen v27 shape (the
    /// **downgrade-bless** path). `weather_ambient_c` has no v27 home and is
    /// dropped — the deliberately lossy direction.
    fn from_current(a: SkyAtmosphere) -> Self {
        Self {
            enabled: a.enabled,
            sun_intensity: a.sun_intensity,
...
struct EntityRecordV14 {
    ...
    time_of_day: Option<TimeOfDay>,
    #[serde(default)]
    sky_atmosphere: Option<SkyAtmosphereV27>,
}
...
#[derive(Clone, Serialize, Deserialize)]
struct EntityRecordV20Gen<M, T = TerrainV23, S = Option<SkyAtmosphereV27>> {
    guid: Uuid,
    name: String,
    parent: Option<Uuid>,
    transform: Transform,
...
pub fn decode(bytes: &[u8]) -> Result<RuntimeLevel> {
    ...
    match header.schema_version {
        ...
        10 => {
            ...
            Ok(RuntimeLevel {
                ...
                entities: v10
                    ...
                    .collect(),
                settings: v10.settings.into_current(),
                geo: Default::default(),
            })
        }
        11 => {
            ...
            Ok(RuntimeLevel {
                ...
                entities: v11
                    ...
                    .collect(),
                settings: v11.settings.into_current(),
                geo: Default::default(),
            })
        }
        12 => {
            ...
            Ok(RuntimeLevel {
                ...
                entities: v12
                    ...
                    .collect(),
                settings: v12.settings.into_current(),
                geo: Default::default(),
            })
        }
        13 => {
            ...
            Ok(RuntimeLevel {
                ...
                entities: v13
                    ...
                    .collect(),
                settings: v13.settings.into_current(),
                geo: Default::default(),
            })
        }
        14 => {
            ...
            Ok(RuntimeLevel {
                ...
                entities: v14
                    ...
                    .collect(),
                settings: v14.settings.into_current(),
                geo: Default::default(),
            })
        }
        15 => {
            ...
            Ok(RuntimeLevel {
                ...
                entities: v15
                    ...
                    .collect(),
                settings: v15.settings.into_current(),
                geo: Default::default(),
            })
        }
        16 => {
            ...
            Ok(RuntimeLevel {
                ...
                entities: v16
                    ...
                    .collect(),
                settings: v16.settings.into_current(),
                geo: Default::default(),
            })
        }
        17 => {
            ...
            Ok(RuntimeLevel {
                ...
                entities: v17
                    ...
                    .collect(),
                settings: v17.settings.into_current(),
                geo: Default::default(),
            })
        }
        18 => {
            ...
            Ok(RuntimeLevel {
                ...
                entities: v18
                    ...
                    .collect(),
                settings: v18.settings.into_current(),
                geo: Default::default(),
            })
        }
        19 => {
            ...
            Ok(RuntimeLevel {
                ...
                entities: v19
                    ...
                    .map(EntityRecordV20::into_runtime)
                    .collect(),
                settings: v19.settings.into_current(),
                geo: Default::default(),
            })
        }
        20 => {
            ...
            Ok(RuntimeLevel {
                ...
                entities: v20
                    ...
                    .map(EntityRecordV20::into_runtime)
                    .collect(),
                settings: v20.settings.into_current(),
                geo: Default::default(),
            })
        }
        21 => {
            ...
            Ok(RuntimeLevel {
                ...
                entities: v21
                    ...
                    .collect(),
                settings: v21.settings.into_current(),
                geo: Default::default(),
            })
        }
        22 => {
            ...
            Ok(RuntimeLevel {
                ...
                entities: v22
                    ...
                    .collect(),
                settings: v22.settings.into_current(),
                geo: Default::default(),
            })
        }
        23 => {
            ...
            Ok(RuntimeLevel {
                ...
                entities: v23
                    .entities
                    .into_iter()
                    .map(EntityRecordV23::into_runtime)
                    .collect(),
                settings: v23.settings.into_current(),
                geo: Default::default(),
            })
        }
        24 => {
            ...
            Ok(RuntimeLevel {
                ...
                entities: v24
                    ...
                    .collect(),
                settings: v24.settings.into_current(),
                geo: v24.geo,
            })
        }
        25 => {
            ...
            Ok(RuntimeLevel {
                ...
                entities: v25
                    .entities
                    .into_iter()
                    .map(EntityRecordV25::into_runtime)
                    .collect(),
                settings: v25.settings.into_current(),
                geo: v25.geo,
            })
        }
        26 => {
            let (v26, _): (SceneFileV26, usize) =
                bincode::serde::decode_from_slice(bytes, bincode_config())
        ...
        27 => {
            ...
            Ok(RuntimeLevel {
                title: v27.title,
                // A per-record LIFT, not an identity. The `26 =>` branch was an
                // identity until VEH2a's own audit found that nothing executed
                // it; this branch is written as the lift it has to be from the
                // day the rung lands, and `a_v27_payload_decodes_at_v28_with_
                // every_v26_field_at_its_own_offset` runs it over real bytes.
                entities: v27
                    .entities
                    .into_iter()
                    .map(EntityRecordV27::into_runtime)
                    .collect(),
        ...
        28 => {
            let (v28, _): (SceneFileV28, usize) =
                bincode::serde::decode_from_slice(bytes, bincode_config())
                    .map_err(|e| SceneError::Decode(format!("v28: {e}")))?;
            Ok(RuntimeLevel {
                title: v28.title,
                entities: v28.entities,
                settings: v28.settings,
                geo: v28.geo,
            })
        }
...
mod tests {
    ...
    fn v8_world_decoration_components_round_trip() {
        ...
        base.settings.render = RenderSettingsRecordV25 {
            exposure: 1.4,
            dither: false,
            bloom_enabled: true,
            bloom_threshold: 0.8,
    ...
    fn v10_scene_reference() -> SceneFileV10 {
        ...
        SceneFileV10 {
            ...
            settings: RuntimeSettingsV25::from_current(RuntimeSettings {
                gravity_2d: Vec2d::new(0.0, -18.0),
                gravity_3d: Vec3d::new(0.0, -9.81, 0.0),
                sim_hz: 90.0,
                render: RenderSettingsRecord {
    ...
    fn v12_atmosphere_is_wider_on_the_wire_than_v11() {
        ...
        let terrained = terrained_entities(&v11);
        assert_eq!(
            lifted.len(),
            v11.len()
                + 61
                ...
                + entities
                    * (WATER_SLOT_BYTES
                        ...
                        + MOVEMENT_SLOT_BYTES
                        + VEHICLE_SLOT_BYTES)
                + materialed * (MATERIAL_BINDING_BYTES + EMISSIVE_INTENSITY_BYTES)
                + terrained * TERRAIN_LAYER_MATERIAL_BYTES
                + geo_anchor_bytes()
                + V26_RENDER_BLOCK_BYTES
                + AMBIENT_WIRE_BYTES,
    ...
    fn v13_clouds_are_wider_on_the_wire_than_v12() {
        ...
        let terrained = terrained_entities(&v12);
        assert_eq!(
            lifted.len(),
            v12.len()
                + expected
    ...
    /// Wire width of the **v28 air temperature** appended to the weather block
    /// (wave VEH3a's audit) — one `f32`, on every entity that carries a sky.
    ///
    /// Beside [`WEATHER_WIRE_BYTES`] rather than folded into it, because that
    /// constant is the price of the **v14** block and a v13 payload lifted to
    /// the current schema pays both.
    const AMBIENT_WIRE_BYTES: usize = 4;
    ...
    fn v14_weather_is_wider_on_the_wire_than_v13() {
        ...
        let terrained = terrained_entities(&v13);
        assert_eq!(
            lifted.len(),
            v13.len()
                + expected
    ...
    fn the_frozen_tile_generation_covers_this_schema() {
        assert_eq!(
            ...
            "the scene schema moved. Generation-1 frozen tiles (TerrainTileFrozenV1, via \
             ...
             ever contained, and v23 appended an entity slot. v24 is the latter case \
    ...
    fn v17_costs_one_byte_per_water_free_entity() {
        ...
        let v16 = SceneFileV16 {
            ...
            entities: dry
                ...
                .collect(),
            settings: RuntimeSettingsV25::from_current(dry.settings),
        };
    ...
    fn v18_costs_one_byte_per_buoyancy_free_entity() {
        ...
        let v17 = SceneFileV17 {
            ...
            entities: sinks
                ...
                .map(EntityRecordV17::from_current)
                .collect(),
            settings: RuntimeSettingsV25::from_current(sinks.settings),
        };
    ...
    fn v19_costs_one_byte_per_voxel_free_entity() {
        ...
        let v18 = SceneFileV18 {
            ...
            entities: solid
                ...
                .map(EntityRecordV18::from_current)
                .collect(),
            settings: RuntimeSettingsV25::from_current(solid.settings),
        };
        let v18_bytes = bincode::serde::encode_to_vec(&v18, bincode_config()).unwrap();
    ...
    // ── schema v24 (Wave G) price terms ───────────────────────────────────

    /// What one splat layer's new material binding costs an entity that carries
    /// a terrain: one `Option` discriminant, four layers.
    ///
    /// Written against [`TERRAIN_LAYERS`] rather than as the literal 4, so a
    /// future layer count moves the price with it.
    const TERRAIN_LAYER_MATERIAL_BYTES: usize = inf_ecs::components::TERRAIN_LAYERS;

    /// How many entities of an encoded level carry a terrain — the multiplier
    /// for [`TERRAIN_LAYER_MATERIAL_BYTES`], counted for the same reason
    /// [`materialed_entities`] is.
    fn terrained_entities(bytes: &[u8]) -> usize {
        RuntimeLevel::decode(bytes)
            .unwrap()
            .entities
            .iter()
    ...
    /// The bytes a **disabled** geo-anchor costs a file (schema v24) — the
    /// file-level constant every lift-price arm now carries.
    ///
    /// **Measured, not written down.** The anchor is a bool, a CRS string, six
    /// f64s, two more strings and a datum name; hand-counting that is exactly the
    /// kind of arithmetic that goes stale the first time somebody adds a field,
    /// and a stale constant here would make every price arm wrong at once while
    /// still looking authoritative.
    fn geo_anchor_bytes() -> usize {
        bincode::serde::encode_to_vec(inf_math::geo::GeoAnchor::default(), bincode_config())
            .expect("the default anchor encodes")
            .len()
    }
    ...
    fn v21_costs_three_bytes_per_ordinary_entity() {
        ...
        let frozen = bincode::serde::encode_to_vec(
            &SceneFileV20 {
                ...
                settings: RuntimeSettingsV25::default(),
            },
            bincode_config(),
        )
        .unwrap();
    ...
    struct MaterialV22Wire {
        ...
        #[allow(dead_code)]
        emissive: Color,
        /// The v26 field, at the slot v26 exists for — declared here so a
        /// material grown WITHOUT a bump leaves bytes this shape cannot account
        /// for, and so a field inserted before it fails to decode.
        #[allow(dead_code)]
    ...
    ///
    /// This is the shape v24 exists for, so the pin has to be able to see it. If
    /// the wire simply reused the frozen `TerrainLayerV23` the pin would decode a
    ...
    /// the `MaterialV22Wire` idiom, at the component wave VEH3a's audit grew.
    ///
    /// Forty-eight fields in declaration order. **Before this existed the pin
    /// named the LIVE `SkyAtmosphere`**, so it could not have caught a field
    /// appended to the sky without a bump — the one shape of drift it is for.
    /// A field added to the live component now leaves bytes this shape cannot
    ...
    type V28EntityShadow<'a> = (&'a RuntimeEntity, Option<u8>);
        ...
        let level = RuntimeLevel {
            ...
            entities: vec![
                RuntimeEntity {
                    ...
                    // A SKY, so the independently declared `SkyAtmosphereWire`
                    // is actually DECODED THROUGH rather than skipped as an
                    // absent `Option` — a pin nothing walks pins nothing.
                    sky_atmosphere: Some(SkyAtmosphere {
                        weather_ambient_c: -7.5,
                        ..hostile_v27_sky()
                    }),
    ...
    fn a_tail_slot_appended_without_a_bump_leaves_bytes_the_pin_refuses() {
        ...
        assert!(
            ...
            "the shadow v25 is not longer than v24, so nothing was appended and \
    ...
    fn v22_costs_one_byte_per_materialed_entity_and_nothing_otherwise() {
        ...
        assert_eq!(
            ...
            "the v22 binding must cost exactly one discriminant byte on a \
             ...
             file-level constant, since `live` is v24)"
        );
        assert_eq!(
            live(&bare).len(),
            frozen(&bare).len()
    ...
    /// What v26's render block costs **the file**, once, whatever it contains:
    /// twenty-one appended fields — six SSR, five exposure, one bloom flag, five
    /// flare, five lens — encoded at their defaults.
    ///
    /// Five of them are `bool`/`u8` (one byte each), sixteen are `f32` (four),
    /// and `flare_ghost_count` is a `u32` that bincode's varint encoding writes
    /// as **one** byte at the default 4: 5 + 64 + 1 = **70**. Measured rather
    /// than asserted from the arithmetic by
    /// [`the_v26_costs_are_what_the_constants_say`], which is the only honest way
    /// to price a varint field.
    const V26_RENDER_BLOCK_BYTES: usize = 70;

    /// What v26's `Material::emissive_intensity` costs an entity that HAS a
    /// material: one `f32`, inside the component — so an entity with no material
    /// pays nothing at all.
    const EMISSIVE_INTENSITY_BYTES: usize = 4;

    /// **The two v26 constants above, measured rather than derived** — the arm
    /// every other v26 price in this file is priced by reference to.
    ///
    /// Deriving them from the field list is exactly the mistake bincode's varint
    /// integers punish: `flare_ghost_count` is a `u32` whose default of 4 encodes
    /// in ONE byte, not four, so a hand-count that says "twenty-one fields, four
    /// bytes each where they are wide" is wrong by three before it starts.
    #[test]
    fn the_v26_costs_are_what_the_constants_say() {
        let plain = v9_rec(Uuid::from_u128(0xFE01), "Marker", None).into_runtime();
        let mut materialed = plain.clone();
        materialed.material = Some(Material::default());
        ...
        let frozen = |e: &RuntimeEntity| {
            bincode::serde::encode_to_vec(
                &SceneFileV25 {
                    schema_version: 25,
                    title: "t".into(),
                    entities: vec![EntityRecordV25::from_current(e.clone())],
                    settings: RuntimeSettingsV25::default(),
                    geo: Default::default(),
                },
                bincode_config(),
        ...
        let block = live(&plain) - frozen(&plain);
        let with_material = live(&materialed) - frozen(&materialed);
        eprintln!(
            "v26 on the wire: {block} B of render block on a level with no material, \
             {with_material} B on one entity that has one"
        );
        assert_eq!(
            block, V26_RENDER_BLOCK_BYTES,
            "the v26 render block costs {block} bytes, not the pinned \
             {V26_RENDER_BLOCK_BYTES}"
        );
        assert_eq!(
            with_material - block,
            EMISSIVE_INTENSITY_BYTES,
            "the emissive intensity costs {} bytes on a materialed entity, not \
             the pinned {EMISSIVE_INTENSITY_BYTES}",
            with_material - block
        );
    }
    ...
    /// The `.inf_mat` the v24 tests bind to a splat layer. A fixed GUID so the
    /// two codec mirrors can be byte-compared.
    fn v24_fixture_layer_material() -> Uuid {
        Uuid::from_u128(0xFA7E_0024)
    }

    /// The **v24** geo-anchor the fixture carries: every field away from its
    /// default, so a hop that produced defaults would be caught.
    ///
    /// The literals must match the editor codec's `v24_fixture_geo` exactly —
    /// the two committed fixtures are byte-compared.
    ...
    ///
    /// Built by downgrading the **v22** reference's lift one rung, so the field
    /// list can never drift from the ladder.
    fn v23_scene_reference() -> SceneFileV23 {
        let v22 = v22_scene_reference();
        let mut entities: Vec<EntityRecordV23> = v22
            .entities
            .into_iter()
            .map(|e| EntityRecordV23::from_current(e.into_runtime()))
            .collect();
    ...
    /// **A NON-DEFAULT v26 RECORD SURVIVES A ROUND TRIP** — every one of the
    /// twenty-two appended fields, set to a value that is not its default, and
    /// compared field-for-field on the way back.
    ///
    /// This is what "an unread field is fine, an unread WIRE field is not" means
    /// operationally: most of these have no consumer until VIS1b, and all of them
    /// are persisted, applied by both hosts and checked here from the day they
    /// land.
    #[test]
    fn v26_render_block_round_trips_field_for_field() {
        ...
        let bytes = encode(&level).unwrap();
        assert_eq!(bytes[0], SCHEMA_VERSION as u8);
        let back = decode(&bytes).expect("a v26 level decodes");
        assert_eq!(back.settings.render, authored);
        assert_eq!(back, level);
    }

    /// The **emissive intensity** survives a round trip, and it is genuinely
    /// per-entity: a second material with no intensity change must come back
    /// unchanged, which a codec writing one value into every slot would fail.
    #[test]
    fn v26_emissive_intensity_round_trips_through_the_codec() {
        let bright = Material {
        ...
        let level = RuntimeLevel {
            ...
            entities: vec![
                RuntimeEntity {
                    material: Some(bright),
                    ..v9_rec(Uuid::from_u128(0xFE10), "Lamp", None).into_runtime()
                },
                RuntimeEntity {
                    material: Some(Material::default()),
                    ..v9_rec(Uuid::from_u128(0xFE11), "Wall", None).into_runtime()
                },
            ],
            settings: RuntimeSettings::default(),
            geo: Default::default(),
        ...
        let back = decode(&bytes).expect("a v26 level decodes");
        assert_eq!(back, level);
        assert_eq!(back.entities[0].material.unwrap().emissive_intensity, 24.0);
        assert_eq!(back.entities[1].material.unwrap().emissive_intensity, 1.0);

        // …and clearing it really changes the bytes.
        let mut dim = level.clone();
        dim.entities[0]
            .material
            .as_mut()
            .unwrap()
    ...
    /// lands next; a count of "how many came back different, and
    /// was each of those exactly the default" is a claim about the *rung* and
    /// fails the day a field is appended without this arm moving.
    ///
    /// **The count is 85 and not 47, and that is exactly why it is a count.**
    /// A v26 file predates BOTH class windows, so downgrading to it loses
    /// VEH2a's forty-seven *and* VEH3a's thirty-eight; this arm moved when the
    /// v28 rung landed, which is the behaviour it was built for. The
    /// rung-LOCAL claim — v28 lost thirty-eight and nothing else — is
    /// `v27_downgrade_is_lossless_except_for_what_v28_added`.
    #[test]
    fn v26_downgrade_is_lossless_except_for_what_v27_added() {
        ...
        assert_eq!(
            ...
            "the v27 rung appended forty-seven tunables and v28 thirty-eight \
             ...
             rather than eighty-five, so a round trip through the frozen v26 \
             shape kept something it cannot express (or the fixture stopped \
    ...
    /// The sixty-two names `VehicleClass` carried at v27, restated **here**
    /// rather than derived from anything (wave VEH3a, on the `V25_CLASS_NAMES`
    /// discipline).
    ///
    ...
    /// live type that had drifted. This is the list a v27 file's sixty-two
    /// `f64`s are, in the order it wrote them: the fifteen of v25, then the
    /// forty-seven VEH2a appended.
    ...
    /// **The v27 `SkyAtmosphere`, re-declared independently** for the
    /// payload written below — the `V27ClassWriter` idiom at the sky slot.
    ///
    /// Forty-seven fields: the live component minus `weather_ambient_c`.
    /// Declared here rather than reusing the codec's own frozen
    /// `SkyAtmosphereV27`, so the bytes this fixture writes say what v27
    /// said rather than what the frozen record currently says.
    #[derive(serde::Serialize)]
    // A wire pin exists to be DECODED THROUGH, not read — the same reason
    // CharacterMovementWire beside it carries this. Its fields ARE the
    // assertion: naming them is what makes the shape independent.
    ...
    /// The frozen v27 sky, copied field-by-field into the INDEPENDENT writer
    /// above — the `V27ClassWriter` route, at the sky slot. The copy is by name,
    /// so the bytes come out in the writer's declaration order.
    ...
    /// is a statement about the v27 layout rather than about whatever the frozen
    ...
    /// The v27 entity record with the class written by [`V27ClassWriter`].
    type V27EntityWriter =
        RuntimeEntityGen<Terrain, Option<V27ClassWriter>, Material, Option<SkyAtmosphereV27Writer>>;
    ...
    struct SceneFileV27Writer {
        schema_version: u32,
        title: String,
        entities: Vec<V27EntityWriter>,
        settings: RuntimeSettings,
        geo: inf_math::geo::GeoAnchor,
    }

    /// **A REAL v27 PAYLOAD DECODES AT v28, AND EVERY v27 FIELD IS WHERE IT WAS**
    /// (wave VEH3a).
    ///
    /// The rung's own arms are in memory: `from_current` then `into_runtime`,
    /// which never touches a byte. The **ladder's** `27 =>` branch is the code a
    /// shipped build runs on every pre-v28 level that already exists — including
    /// all twenty-four this repository commits, until they are re-blessed — and
    /// VEH2a's own audit found that the equivalent `26 =>` branch had been an
    /// identity that no test executed. So this arm exists on the day the rung
    /// lands rather than a wave later.
    ///
    /// Bytes written by the independent v27 writer, fed to the public
    /// [`decode`], and each of the sixty-two checked **by name** against the
    /// value it was written with. A tail spliced one slot early or late, a
    /// frozen record whose fields drifted out of the live prefix's order, or a
    /// `27 =>` branch that forgot to lift, all land here as a named field
    ...
    fn a_v27_payload_decodes_at_v28_with_every_v27_field_at_its_own_offset() {
        ...
        let live = RuntimeEntity {
            ...
            character_movement: Some(v23_fixture_movement()),
            terrain: Some(fixture_terrain()),
            // A SKY as well, so the same payload proves the forty-seven v27
            ...
            // and the arm below is where that is measured.
            sky_atmosphere: Some(hostile_v27_sky()),
            ..v9_rec(Uuid::from_u128(0xFE30), "Car", None).into_runtime()
        };
        // A second entity with NO class, so a ladder that wrote one value into
        // every slot cannot pass.
        let bare = v9_rec(Uuid::from_u128(0xFE31), "Marker", None).into_runtime();
        let writer = |r: &RuntimeEntity| -> V27EntityWriter {
        ...
        for (name, value) in got.settings() {
            if V27_CLASS_NAMES.contains(&name) {
                ...
                assert_eq!(
                    value, want[name],
                    "`{name}` was authored at v27 and came back as {value} rather \
                     than {} — the sixty-two no longer occupy their v27 offsets",
                    want[name]
                );
            } else {
                assert_eq!(
        ...
        assert_eq!(
            kept, 62,
            "{kept} of the hundred settings are v27 names; the frozen list and \
             the live door no longer agree about which sixty-two v27 wrote"
        );
        ...
        assert_eq!(
            ...
            live,
            "the v27 rung changed something that is not the vehicle class"
        );
        assert_eq!(encode(&back).unwrap()[0], SCHEMA_VERSION as u8);
    }

    /// A [`SkyAtmosphere`] with **every v27 field off its default**, so a rung
    /// that dropped one, reordered two or lifted a constant is visible.
    ///
    /// `weather_ambient_c` is deliberately left at the live default: it is the
    /// one field a v27 payload cannot carry, and
    /// `the_v27_sky_rung_loses_exactly_the_air` is where that is measured.
    ...
    /// **The downgrade direction for v28's SECOND row** — the frozen v27 entity
    /// record loses the air temperature and **nothing else about the sky**
    /// (wave VEH3a's audit).
    ///
    /// The mutation: dropping any one of the forty-seven from
    /// `SkyAtmosphereV27::into_current` — that field comes back at its Ring-0
    /// default and the equality below names it.
    #[test]
    fn the_v27_sky_rung_loses_exactly_the_air() {
        let authored = SkyAtmosphere {
            // 3.5 °C: a cold morning nothing in the weather block could have
            // said before this row existed, and not a default of anything.
        ...
        let live = RuntimeEntity {
            sky_atmosphere: Some(authored),
            ..v9_rec(Uuid::from_u128(0xFE36), "Sky", None).into_runtime()
        };
        let back = EntityRecordV27::from_current(live.clone()).into_runtime();
        assert_ne!(back, live, "the air must really be what is lost");
        let got = back.sky_atmosphere.expect("the sky survives the rung");
        assert_eq!(
            got.weather_ambient_c,
            SkyAtmosphere::default().weather_ambient_c,
        ...
        // …and the rest of the record came through untouched.
        assert_eq!(
            RuntimeEntity {
                sky_atmosphere: live.sky_atmosphere,
                ..back
            },
            live,
            "the v27 sky rung changed something that is not the sky"
        );
    }

    /// The air costs **four bytes on an entity that carries a sky** and nothing
    /// on one that does not — the second half of v28's price, measured the way
    /// `v28_costs_304_bytes_on_a_class_and_nothing_on_anything_else` measures
    /// the first.
    #[test]
    fn v28_costs_four_bytes_on_a_sky_and_nothing_on_anything_else() {
        let bare = v9_rec(Uuid::from_u128(0xFE37), "Marker", None).into_runtime();
        let with = RuntimeEntity {
            sky_atmosphere: Some(hostile_v27_sky()),
            ..bare.clone()
        };
        let a = bincode::serde::encode_to_vec(&bare, bincode_config()).unwrap();
        let b = bincode::serde::encode_to_vec(&with, bincode_config()).unwrap();
        let frozen = EntityRecordV27::from_current(with.clone());
        let c = bincode::serde::encode_to_vec(&frozen, bincode_config()).unwrap();
        assert_eq!(
            b.len() - c.len(),
            4,
            "the v28 air costs {} bytes on a sky-carrying entity, not one f32",
            b.len() - c.len()
        );
        // …and the frozen record's bytes are a strict PREFIX of the live one's
        // up to the sky, which is what "appended at the block's tail" means.
        let bare_frozen = EntityRecordV27::from_current(bare.clone());
        let d = bincode::serde::encode_to_vec(&bare_frozen, bincode_config()).unwrap();
        assert_eq!(
            a, d,
            "an entity with no sky and no class is not byte-identical across the v28 rung"
        );
    }

    /// **The downgrade direction for v28** — the frozen v27 entity record loses
    /// the thirty-eight VEH3a tunables and nothing else (wave VEH3a).
    ///
    /// A **COUNT**, not a field list, on `v26_downgrade_is_lossless_except_for_
    /// what_v27_added`'s own discipline: a list stops covering whatever lands
    /// next, and a count fails the day a hundred-and-first field is appended
    /// without a rung.
    #[test]
    fn v27_downgrade_is_lossless_except_for_what_v28_added() {
        let live = RuntimeEntity {
            ...
            character_movement: Some(v23_fixture_movement()),
            terrain: Some(fixture_terrain()),
            ..v9_rec(Uuid::from_u128(0xFE32), "Car", None).into_runtime()
        };
        let back = EntityRecordV27::from_current(live.clone()).into_runtime();
        assert_ne!(back, live, "the thirty-eight must really be what is lost");
        ...
        assert_eq!(
            lost, 38,
            "the v28 rung appended thirty-eight tunables; {lost} of them survived \
             a round trip through the frozen v27 shape or the fixture stopped \
             differing from the defaults"
        );
        ...
        assert_eq!(
            patched, live,
            "the v28 downgrade touched something that is not the vehicle class"
        );
    }

    /// **THE TRIPWIRE, ARMED**: a tunable `VehicleTuning` knows and
    /// `VehicleClass` does not is silently dropped on every round trip
    /// (wave VEH3a).
    ///
    /// `VehicleClass::set` routes through `to_tuning`/`from_tuning`, so the
    /// failure mode of a half-landed field is not a compile error and not a
    /// decode error — it is an authored number that quietly becomes the default
    /// the next time anything touches the class. Ring 0 owns the general form
    /// (`the_class_and_the_tuning_are_the_same_hundred_numbers`); this is the
    /// **codec's** half, over the WIRE, for the thirty-eight this rung landed:
    /// each one authored to a value that is not its default, encoded, decoded,
    /// and read back by name.
    #[test]
    fn every_v28_tunable_survives_the_wire_by_name() {
        let mut authored = VehicleClass::default();
        let default = VehicleClass::default();
        let mut moved: Vec<&str> = Vec::new();
        for (i, (name, _)) in default.settings().into_iter().enumerate().skip(62) {
            // Distinct per field, finite, and never equal to a default: the
            // defaults in this tail are 0.0, 0.06, 0.15, 0.18, 0.25, 0.3, 0.7,
            // 0.9, 4.0, 15.0, 85.0, 120.0, 420.0, 4500.0 and 9000.0, none of
            // which is of the form 1 000 000 + i.
            assert!(
        ...
        assert_eq!(moved.len(), 38, "the v28 tail is not thirty-eight long");

        let level = RuntimeLevel {
            title: "V28".into(),
            entities: vec![RuntimeEntity {
                vehicle_class: Some(authored),
                ..v9_rec(Uuid::from_u128(0xFE33), "Car", None).into_runtime()
            }],
            settings: RuntimeSettings::default(),
            geo: Default::default(),
        };
        ...
        let back = decode(&bytes).expect("a v28 level decodes");
        let got = back.entities[0].vehicle_class.expect("the class survived");
        for (i, (name, value)) in got.settings().into_iter().enumerate().skip(62) {
            assert_eq!(
                value,
                1_000_000.0 + i as f64,
                "`{name}` did not survive the wire — a v28 tunable that reaches \
                 `names()` but not `from_tuning` is dropped silently, which is \
                 the one failure this rung has to be unable to have"
            );
        }
        // …and the whole class round-tripped, which the loop above alone would
        // not prove (it reads only the tail).
        assert_eq!(got, authored);
    }

    /// **THE WIRE COST OF THE v28 TAIL, MEASURED** (wave VEH3a).
    ///
    /// Not derived from `38 × 8`: encoded, differenced, and asserted — the
    /// `v25_costs_one_discriminant_per_entity_and_no_more` discipline. An entity
    /// with no class pays **nothing**, because the `Option` tag it already had
    /// is the whole slot.
    #[test]
    fn v28_costs_304_bytes_on_a_class_and_nothing_on_anything_else() {
        let bare = v9_rec(Uuid::from_u128(0xFE34), "Marker", None).into_runtime();
        let with = RuntimeEntity {
            vehicle_class: Some(fixture_vehicle_class()),
            ..bare.clone()
        };
        let a = bincode::serde::encode_to_vec(&bare, bincode_config()).unwrap();
        let b = bincode::serde::encode_to_vec(&with, bincode_config()).unwrap();
        // A hundred f64 plus the discriminant the slot always cost.
        assert_eq!(
            b.len() - a.len(),
            800,
            "a present class costs {} bytes over an absent one, not the hundred \
             fixed-width f64 a bincode standard config writes",
            b.len() - a.len()
        );

        // And the v28 half of that, against the frozen v27 shape.
        let frozen = EntityRecordV27::from_current(with.clone());
        let c = bincode::serde::encode_to_vec(&frozen, bincode_config()).unwrap();
        assert_eq!(
            b.len() - c.len(),
            304,
            "the v28 tail costs {} bytes per class-carrying entity, not 38 × 8",
            b.len() - c.len()
        );
        // …and the frozen record's bytes are a strict PREFIX of the live one's,
        // which is what "appended at the tail" means.
        assert_eq!(&b[..c.len()], &c[..], "the v28 tail is not at the tail");
    }
    ...
    type V26EntityWriter =
        RuntimeEntityGen<Terrain, Option<V25ClassWriter>, Material, Option<SkyAtmosphereV27Writer>>;
    ...
    ///
    /// Asserted as a property — patch the one field back and compare by
    /// `PartialEq` — rather than as a field list, which would silently stop
    /// covering whatever lands next.
    #[test]
    fn v25_downgrade_is_lossless_except_for_what_v26_added() {
        ...
        let round = RuntimeSettingsV25::from_current(settings).into_current();
    ...
    fn v25_costs_one_discriminant_per_entity_and_no_more() {
        ...
        for n in [1usize, 4, 16] {
            assert_eq!(
                live(n),
                frozen(n) + n + V26_RENDER_BLOCK_BYTES,
                "v25 costs {} bytes over {n} entities, not {n} plus v26's block",
                live(n) - frozen(n)
            );
        }
    }

    /// **What the two class tails COST, in bytes, measured against the frozen
    /// v26 shape** — 680 per entity that carries a class and zero for every
    /// other.
    ///
    /// The ladder's own claim, checked rather than derived. `(47 + 38) × 8` is
    /// the arithmetic; what this arm adds is that the eighty-five really are
    /// eight
    ...
    ///
    /// It measures against **v26**, which is the frozen shape it holds, so it
    /// necessarily grew with the v28 rung. The v28 half on its own is
    /// `v28_costs_304_bytes_on_a_class_and_nothing_on_anything_else`, which
    /// measures against the frozen v27 shape and will not move when a later
    /// rung lands.
    #[test]
    fn the_class_tails_cost_680_bytes_a_class_and_nothing_at_all_without_one() {
        ...
        for n in [1usize, 4, 16] {
            assert_eq!(
                size(&carrying, n),
                frozen(&carrying, n) + 680 * n,
                "the v27 + v28 tails cost {} bytes over {n} entities WITH a class",
                size(&carrying, n) - frozen(&carrying, n)
            );
            assert_eq!(
                size(&bare, n),
                frozen(&bare, n),
                "an entity with NO class paid {} bytes for the two class tails",
    ...
    /// path, walked only under `INF_BLESS_FIXTURES=1`.
    #[test]
    fn bless_scene_v23_fixture() {
        if std::env::var("INF_BLESS_FIXTURES").as_deref() != Ok("1") {
            return;
        }
        let bytes = bincode::serde::encode_to_vec(v23_scene_reference(), bincode_config()).unwrap();
        assert_eq!(bytes[0], 23);
        let path = Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/scene_v23.inf_lvl");
        std::fs::write(&path, &bytes).unwrap();
        eprintln!("blessed {} ({} bytes)", path.display(), bytes.len());
    }
    ...
    fn v23_fixture_is_reproducible_and_genuinely_v23() {
        ...
        let rebuilt =
            bincode::serde::encode_to_vec(v23_scene_reference(), bincode_config()).unwrap();
        assert_eq!(
            rebuilt, bytes,
    ...
    /// **The "old bytes load forever" gate for the v24 bump.**
    ///
    /// A committed v23 payload still loads, keeps everything v23 could express,
    /// and lifts with **no geo-anchor and no per-layer material** — which is
    /// exactly what a v23 level was.
    #[test]
    fn v23_loads_and_lifts_without_an_anchor_or_layer_materials() {
        let bytes = std::fs::read(
            Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/scene_v23.inf_lvl"),
        )
        .expect("committed v23 fixture present");
        assert_eq!(bytes[0], 23);
        let level = decode(&bytes).expect("a v23 level decodes");
        assert!(!level.entities.is_empty());

        // Nothing in a v23 file could have authored either v24 addition.
        assert!(
            !level.geo.enabled,
            "a v23 file has no anchor, so the level must come up un-georeferenced"
        );
        assert_eq!(level.geo, Default::default());
        for e in &level.entities {
        ...
        // …and everything v23 COULD express survived the hop, or "it loads"
        // would be satisfied by a decoder that dropped the payload.
        let hero = level
            .entities
            .iter()
    ...
    /// **The v24 additions round-trip through the whole codec** — including the
    /// new decode arm, which only a payload stamped v24 exercises.
    ///
    /// Un-fix mutation: drop `geo` from `SceneFileV24` and the anchor comes back
    /// disabled; drop `material` from `TerrainLayer` and the layer bindings do.
    #[test]
    fn v24_anchor_and_layer_materials_round_trip_through_the_codec() {
        let mut terrain = fixture_terrain();
        // Bind two of the four layers, and deliberately leave two unbound: a
        // codec that wrote the same value into every slot would pass a
        // one-layer check.
        terrain.layers[0].material = Some(v24_fixture_layer_material());
        terrain.layers[2].material = Some(Uuid::from_u128(0xFA7E_0025));

        let level = RuntimeLevel {
            title: "V24".into(),
            entities: vec![RuntimeEntity {
                terrain: Some(terrain),
                ..v9_rec(Uuid::from_u128(0xFD24), "Ground", None).into_runtime()
            }],
            settings: RuntimeSettings::default(),
            geo: v24_fixture_geo(),
        };
        ...
        let bytes = encode(&level).unwrap();
        assert_eq!(bytes[0], SCHEMA_VERSION as u8);
        let back = decode(&bytes).expect("a v24 level decodes");
        assert_eq!(back, level);

        // The anchor came back field for field, not merely enabled.
        assert_eq!(back.geo, v24_fixture_geo());
        assert!((back.geo.origin_easting_m - 491_000.0).abs() < 1e-9);
        assert_eq!(back.geo.vertical_datum, "EGM2008");

        // Each layer binding landed in ITS OWN slot.
        ...
        assert_eq!(t.layers[2].material, Some(Uuid::from_u128(0xFA7E_0025)));
        assert_eq!(t.layers[3].material, None, "layer 3 was never bound");
        // …and the layers' other fields did not move with it.
        assert_eq!(
            t.layers[0].albedo,
            level.entities[0].terrain.as_ref().unwrap().layers[0].albedo
        );

        // Clearing them really changes the bytes (the fields are persisted, not
        // inferred).
        let mut bare = level.clone();
        bare.geo = Default::default();
        if let Some(t) = bare.entities[0].terrain.as_mut() {
    ...
    /// The **downgrade** direction for v24: projecting a live record onto the
    /// frozen v23 shape loses the per-layer material bindings and **nothing
    /// else**.
    ///
    /// Asserted as a property — the record is compared by `PartialEq` after
    /// patching back the one field v23 cannot carry — rather than as a field
    /// list, which would silently stop covering a slot added later.
    #[test]
    fn v23_entity_downgrade_is_lossless_except_for_the_layer_materials() {
        let mut terrain = fixture_terrain();
        terrain.layers[1].material = Some(v24_fixture_layer_material());
        let live = RuntimeEntity {
            terrain: Some(terrain),
            character_movement: Some(v23_fixture_movement()),
            ..v9_rec(Uuid::from_u128(0xFD25), "Ground", None).into_runtime()
        };

        let round = EntityRecordV23::from_current(live.clone()).into_runtime();
        assert_ne!(round, live, "the binding must really be lost");
        // Patch back exactly the one thing v23 cannot say, and the two records
        // must then be identical.
        let mut patched = round.clone();
        if let Some(t) = patched.terrain.as_mut() {
            t.layers = live.terrain.as_ref().unwrap().layers;
        }
        assert_eq!(
            patched, live,
            "the v23 downgrade lost something other than the layer materials"
        );
        // And the loss is exactly the bindings, not the whole layer.
        let rt = round.terrain.as_ref().unwrap();
        assert!(rt.layers.iter().all(|l| l.material.is_none()));
        assert_eq!(
            rt.layers[1].albedo,
            live.terrain.as_ref().unwrap().layers[1].albedo
        );
        assert_eq!(
            rt.layers[1].tex_scale,
            live.terrain.as_ref().unwrap().layers[1].tex_scale
        );
    }

    /// What v24 costs: the anchor once per FILE, and one discriminant per splat
    /// layer on an entity that carries a terrain — nothing at all on one that
    /// does not.
    #[test]
    fn v24_costs_an_anchor_per_file_and_four_bytes_per_terrain() {
        let plain = v9_rec(Uuid::from_u128(0xFD26), "Marker", None).into_runtime();
        let live = |e: &RuntimeEntity| {
            encode(&RuntimeLevel {
                title: "t".into(),
                entities: vec![e.clone()],
        ...
        let frozen = |e: &RuntimeEntity| {
            bincode::serde::encode_to_vec(
                &SceneFileV23 {
                    schema_version: 23,
                    title: "t".into(),
                    entities: vec![EntityRecordV23::from_current(e.clone())],
                    settings: RuntimeSettingsV25::default(),
                },
                bincode_config(),
            )
        ...
        assert_eq!(
            live(&plain).len(),
            frozen(&plain).len() + V26_RENDER_BLOCK_BYTES + geo_anchor_bytes() + VEHICLE_SLOT_BYTES,
            "an entity with no terrain must pay nothing for the layer bindings — \
             the field is inside the component"
        );

        // One with a terrain pays four more: one discriminant per splat layer.
        let ground = RuntimeEntity {
            terrain: Some(fixture_terrain()),
            ..plain.clone()
        };
        assert_eq!(
        ...
        // …and a CARRIED binding costs a great deal more than its discriminant,
        // so the four bytes above really are the empty slots.
        let mut bound = ground.clone();
        if let Some(t) = bound.terrain.as_mut() {
            t.layers[0].material = Some(v24_fixture_layer_material());
        }
        assert!(
            live(&bound).len() > live(&ground).len() + 8,
            "a carried layer binding is {} bytes, which is not a GUID",
            live(&bound).len() - live(&ground).len()
        );
    }
    ...
    /// new ceiling.
    #[test]
    fn a_v29_payload_is_refused_by_name() {
        let level = RuntimeLevel {
            title: "Future".into(),
