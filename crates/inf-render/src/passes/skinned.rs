...
pub struct SkinnedMeshNode {
    ...
    /// Empty `@group(2)` (the material seam).
...
impl SkinnedMeshNode {
    pub fn new(gpu: &GpuContext, view_bgl: &wgpu::BindGroupLayout) -> Self {
        ...
        // @group(2) material seam — empty for now.
        let empty_bgl = gpu
            .device
            .create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
                label: Some("skinned-material-seam"),
                entries: &[],
            });
        let empty_bg = gpu.device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("skinned-material-seam"),
            layout: &empty_bgl,
            entries: &[],
        });
