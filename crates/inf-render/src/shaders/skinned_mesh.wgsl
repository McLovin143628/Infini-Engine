...
fn fs(in: VsOut) -> @location(0) vec4<f32> {
    ...
    //
    // **AND THE COVERAGE MAY BE A TEXTURE** (wave OUTFIT1 AUDIT, carried item
    // 164). Until this line the alpha it read was `in.color.a` alone — the
    // instance's constant — so a masked surface could only be entirely there or
    // entirely gone, and a cut-out that lives in a MAP could not exist on this
    // path at all. That is why the island's two MetaHumans wore their hair as
    // solid pale ribbons: the cards are `BLEND_Masked` with a coverage atlas,
    // and the atlas had nowhere to be read. The base colour's own alpha channel
    // is where this engine keeps a masked material's coverage
    // (`role_to_planes`' note on `opacity`), and `vt_sample_color` is the door
    // that reads it — the same slot, the same uv and the same derivatives
    // `vt_surface` uses below, so the mask and the shading cannot disagree
    // about which texel a fragment is.
    //
