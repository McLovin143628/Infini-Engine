...
// Aerial perspective + height fog on lit geometry (P17.2) — split out of
// `atmosphere_lut.wgsl` in P18.4 because, unlike the LUT samplers, this half reads
// the `View` uniform. The GI probe march (`gi_probes.wgsl`) is a compute pass with
// no view uniform at all and needs only `atmos_sample_skyview`, so composing the
// whole library there would fail to parse. Included by
// `passes::atmosphere_apply_source()` from every pass that HAS a view.
//
