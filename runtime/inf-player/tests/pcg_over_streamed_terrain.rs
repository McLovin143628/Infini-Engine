...
fn the_paged_footprint_follows_the_volumes_rather_than_their_bounding_box() {
    ...
    // One volume at the origin, one far away in +X. Their bounding box spans the
    // whole authored world; their own regions are two small squares.
    let mut world = EcsWorld::default();
    let t = world.spawn_with_guid(TERRAIN_GUID, "Terrain", None);
    let mut terrain = Terrain::configured(RESOLUTION, METERS_PER_SAMPLE);
    ...
    world.world_mut().entity_mut(t).insert(terrain);
    world.world_mut().entity_mut(t).insert(Transform::default());
    for (i, x) in [0.0f64, SPAN - 8.0].iter().enumerate() {
        ...
        world
            .world_mut()
            .entity_mut(v)
            .insert(Transform::from_translation(glam::DVec3::new(*x, 0.0, 0.0)));
    }
    world.propagate();
    ...
    // Four tiles on a side ⇒ 16 level-0 tiles authored. Two 8 m squares at
    // opposite corners cannot need all of them.
    let total_level0 = 16;
    assert!(
        paged > 0 && paged < total_level0,
        "paged {paged} of {total_level0} level-0 tiles for two 8 m squares at \
         opposite corners — a bounding-box want set would have taken every one"
    );
    eprintln!("IB-1 footprint: {paged} of {total_level0} level-0 tiles for two distant volumes");
}
