...
impl RuntimeSim {
    ...
    pub fn state_bytes(&mut self) -> Vec<u8> {
        ...
        // WPN2c appends the BRASS, last, on the fifteen above's argument with
        // one twist of its own: a casing is a body with a velocity and a contact
        // count, and two hosts that disagreed about it would put different brass
        // on the floor and — because the pool is what the draw pass reconciles
        // its entities against — would spawn and despawn different ENTITIES,
        // which is a divergence in the snapshot section fifteen rows up.
        //
        // **The position is frozen** and `projector_mirror`'s `SECTIONS`
        // allowlist is extended in the SAME commit. **EMPTY when no casing
        // exists**, which is the half that keeps every trace committed before
    ...
    fn fixed_step(&mut self) {
        ...
        self.venue_audio = inf_ecs::venue::sync_venue_audio(&mut self.world);
    ...
    fn step_camera(&mut self, dt: f64) {
        ...
        if let Some(subject) = self.camera_subject {
            ...
            // the director's stack. (MIRROR of `SimSession::fixed_step`.)
    ...
    fn audio_step(&mut self) {
        ...
        //    Inert on a level with no dispatcher: one `get_resource` and an
        //    empty slice. (MIRROR of the other host's fixed step.)
        let (dworld, dcmds) = (&self.world, &mut self.audio_cmds);
        ...
        for (guid, src, pos) in autoplay {
            ...
            if src.occlusion && src.spatial {
                cmd.occlusion_gain =
                    portal(&self.world, &mut self.bridge3d, listener_pos, pos).gain;
            }
            self.audio_cmds.push(AudioCommand::Play(cmd));
        }
        ...
        for (i, emitter) in occ {
            let gain = portal(&self.world, &mut self.bridge3d, listener_pos, emitter).gain;
    ...
    fn fire_weapon_audio(&mut self, hits: &[inf_physics::d3::WeaponHit]) {
        ...
        // The borrow split: the fenced block reads the world and writes the
        // queue, and both are fields of this struct.
        let Self {
            world, audio_cmds, ..
        } = self;
        let world: &EcsWorld = world;
