...
| **The `audio` phase with the vehicle layer stack** (wave VEH3e: 64 driven cars, **all 64 voiced** — the worst case the tier rule prevents, 694.6 commands a step; release 0.0620 ms, dev 0.0940; with the realistic ONE voiced car among 63 voiceless, 10.85 commands a step, release 0.0177 ms; min of five rounds of 60 warmed steps) | `AUDIO_STEP_BUDGET_MS` | 1.0 ms | `inf-player` · `tests/veh3e_gate.rs` (`sixty_four_cars_cost_what_they_print`) |
| **The vehicle roster's catalogue** (wave VEH3f: `VehicleDefs::merge_toml` over the committed roster -- 153 rows and two trailers, every class default applied before its row, every key refused by name if unknown; min of five; release **4.57 ms** alone and **6.51 ms** with the rest of the gate beside it, dev **12.1 ms**) | `ROSTER_LOAD_BUDGET_MS` | 10.0 ms | `inf-player` · `tests/veh3f_gate.rs` (`the_catalogue_loads_in_microseconds`) |
| **The `vehicle` phase at 64 MIXED-CLASS cars** (wave VEH3f: every other wheeled road row of the roster, 14 classes, against 64 of the default row -- VEH3e's baseline shape; min of five rounds of 40 warmed steps; release **0.2869 ms** against 0.2590 (**1.11x**), dev **0.3743** against 0.3322 (1.13x)) | `VEHICLE_STEP_BUDGET_MS` | 0.5 ms | `inf-player` · `tests/veh3f_gate.rs` (`sixty_four_mixed_class_cars_cost_what_they_print`) |
| **The `vehicle` phase at 18 AIR AND SEA CRAFT** (wave VEH3g: the roster's five aeroplanes, five helicopters and eight hulls at once -- wing, rotor, hull, planing and sail models -- against 18 of the roster's sedans; min of five rounds of warmed steps; release **0.0354 ms** against 0.0932 (**0.38x**, 1.97 us a craft), dev 0.0506 against 0.1192 (0.42x). A craft casts no wheel rays unless it has gear, which is why it is cheaper than a car) | `VEHICLE_STEP_BUDGET_MS` | 0.5 ms | `inf-player` · `tests/veh3g_gate.rs` (`eighteen_air_and_sea_craft_cost_what_they_print`) |
| **The player projector over 64 DCC CAR SHELLS** (wave VEH3f.2b: 64 island saloons wearing `shell_sedan` -- 32 drawn meshes a car, body + parts + tyres + rims, 17 876 LOD-0 triangles a car submitted as meshlet DAGs -- against 64 of the same hull as boxes and 64 of the calibration sedan's committed art fallback; min of five; release **0.485 ms** / boxes 0.214 / art fallback 0.230, dev 0.763 / 0.364 / 0.362; the engine's 1080p GPU frame over the lot, min of 30, release 2.274 / 1.615 / 2.097 ms) | `SHELL_PROJECTION_BUDGET_MS` (gate-local, as VEH3f.2a's `ART_PROJECTION_BUDGET_MS`) | 2.0 ms | `inf-player` · `tests/veh3f2b_gate.rs` (`sixty_four_shell_cars_cost_what_they_cost`) |
| **Cell streaming at a jet's speed** (wave VEH3g: VEH2c's speed table extended to 120 m/s, the Luxor on its approach, over the FIXTURE island: 0 blocking loads, the closest a cell arrived to the source 252.9 m, the cell one cell-length ahead on the flight line missing on 0 steps; mean step dev 7.60 ms) | `STREAMED_STEP_BUDGET_MS` | 4.0 ms (asserted at the mandate's 60 m/s only) | `inf-player` · `tests/island_gate.rs` (`streaming_holds_at_aircraft_speed_and_the_table_says_what_it_costs`, arm (e)) |
...
The `vehicle` row was **re-priced again at wave VEH3a** and the constant still did not move. The tyre now casts **four rays a wheel** (the footprint's corners, blended to one contact and a bilinear normal), evaluates the magic formula per axis, reads a surface map at every contact and integrates a temperature: **0.1277 → 0.3971 ms at 64 cars dev, 0.3543 ms release** (6.21 / 5.54 µs a car), measured by `veh3a_gate::the_vehicle_phase_costs_what_it_prints` as the minimum of five over a warmed fixture with a control. Four times the casts for roughly three times the phase — the rays are the cost, again. The headroom is now ~1.3× dev / ~1.4× release, and §8 keeps the constant where it is. **The first CI run of that arm asserted a dev build on three shared runners** (0.52 / 0.86 / 0.87 ms, all over the ceiling) because it was written without the row's own conditioning; it now reports everywhere and asserts only under `cargo test --release` off CI, like every other row in this table. A budget arm against a §8 constant never asserts in dev or on CI.
