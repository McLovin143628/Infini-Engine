...
import { app, assets, dcc, layouts, pcg, script, sim, terrain, viewport } from "../ipc";
...
/**
 * **The InfiniScript commands' half of the seam** (SCRIPT2b audit).
 *
 * The wave armed both ends of the diagnostics wire and neither of the wire:
 * the Rust arm exercises `check_text`, one call below the `#[tauri::command]`,
 * and the TS arm exercises the converter over a DTO built by hand. What crosses
 * the boundary — the command NAME and the argument KEYS — was measured nowhere,
 * and a typo in either is a runtime "command not found" or a missing-key
 * deserialize error with no test between it and the author.
 *
 * This pins the frontend half, which is the half a typo lives in. The backend
 * half is pinned by two other things that already exist: `ScriptDiagnosticDto`
 * is generated by the ts-rs harness and drift-checked in CI, so the payload's
 * field names and types cannot move alone, and `commands/mod.rs`'s
 * `invoke_handler` list is what makes the names resolvable at all. What still
 * has no arm anywhere is the two ends actually talking; that needs a Tauri
 * integration harness this repo does not have, and it is carried by name rather
 * than implied to be covered.
 *
 * `path` is deliberately `null` and not absent when the caller omits it: the
 * command's parameter is `Option<String>`, and `{ path: null }` and no key at
 * all both deserialize to `None`, but only one of them is a value this test can
 * see.
 */
test("the script commands carry their names and argument keys", async () => {
  const calls: Array<[string, Record<string, unknown>]> = [];
  mockIPC((cmd, args) => {
    calls.push([cmd, args as Record<string, unknown>]);
    if (cmd === "script_check") return [];
    if (cmd === "script_emit_class") return 'actor "Door"\n';
  });

  await expect(script.check('actor "Door"\n', "C:/Content/Scripts/Door.infini")).resolves.toEqual(
    [],
  );
  await script.check("actor \"Unsaved\"\n");
  await expect(script.emitClass("11112222-3333-4444-5555-666677778888")).resolves.toBe(
    'actor "Door"\n',
  );
  ...
  expect(calls[0][1]).toEqual({
    text: 'actor "Door"\n',
    path: "C:/Content/Scripts/Door.infini",
  });
  expect(calls[1][1]).toEqual({ text: 'actor "Unsaved"\n', path: null });
  expect(calls[2][1]).toEqual({ assetId: "11112222-3333-4444-5555-666677778888" });
});
