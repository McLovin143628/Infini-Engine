...
mod tests {
    ...
    /// *removed* from the live record silently leaves a tail. Paired with the
    /// asymmetric arm below, which catches the opposite mistake.
    #[test]
    fn a_v21_payload_decodes_consuming_every_byte() {
        let file = SceneFile {
            ...
            entities: vec![
                EntityRecord {
                    ik_target: Some(v21_fixture_ik_target()),
                    ..v9_base(uuid::Uuid::from_u128(0xFD10), "A", None)
                        .into_v10()
                        .into_v11()
                        .into_v12()
                        .into_v13()
                ...
                v9_base(uuid::Uuid::from_u128(0xFD11), "B", None)
                    .into_v10()
                    .into_v11()
                    .into_v12()
                    .into_v13()
        ...
        let bytes = bincode::serde::encode_to_vec(&file, bincode_config()).unwrap();
        let (_, consumed): (SceneFile, usize) =
    ...
    ///
    ///  1. the shadow-v22 bytes are LONGER, so a slot really was added;
    ///  2. decoding them as a `SceneFile` leaves bytes on the floor, which is the
    ///     signature the shape pin refuses. If a future append ever came out
    ...
    fn a_tail_slot_appended_without_a_bump_is_caught_by_the_shape_pin() {
        let file = SceneFile {
            schema_version: SCHEMA_VERSION,
            title: "Shadow".into(),
            entities: vec![v9_base(uuid::Uuid::from_u128(0xFE10), "A", None)
                .into_v10()
                .into_v11()
                .into_v12()
                .into_v13()
        ...
        let v21 = bincode::serde::encode_to_vec(&file, bincode_config()).unwrap();
        // Bytes rather than a shadow RECORD: the record's 47 fields would have to
        // be re-declared to append one, and a re-declaration that drifted from the
        // live shape is exactly what this test exists to catch. The appended
        // `None` discriminant is the cheapest possible new slot, and therefore the
        // hardest case for a pin to notice.
        ...
        let (back, consumed): (SceneFile, usize) =
