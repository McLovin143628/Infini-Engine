...
///   shape and [`LevelSettings`] is untouched, so this is once again the
///   [`EntityRecordV10`] *shape* of bump (a new slot at the tail), not the
...
///   was.
///
///   **The component references no asset**, which is the whole reason it can be
///   this cheap. What breaks is the mesh already on the entity ([`MeshRef`]),
///   and the chunk set is *derived from that mesh at cook time* — a
///   `.inf_fracture` whose GUID is a pure function of the mesh's, exactly as a
///   `.inf_vmesh`'s is. So v20 adds **no** new edge to the cook's dependency
///   closure (the `MeshRef.asset` edge already pulls the mesh in), there is no
///   fracture reference to leave dangling, and no dangling-reference advisory to
///   write. An entity with a `Destructible` and no mesh is a cook *advisory*,
///   not a broken link.
///
///   **This is Phase 22's ONLY bump**, and the five fields are frozen as
///   shipped: P22.3 (runtime destruction) and P22.4 (debris at scale) must both
///   fit inside them. `docs/memos/p22-strength.md` argues why they suffice and
///   lists the things that look like missing fields but belong in the
///   `.inf_fracture` (which versions itself) or in per-tier settings.
///
///   The wire price to a level where nothing breaks is **one discriminant byte
...
///   once — `ik_target` ([`IkTarget`]), `cloth_sim` ([`ClothSim`]) and
///   `hair_guides` ([`HairGuides`]). No component changed shape and
///   [`LevelSettings`] is untouched, so this is once again the
...
///   level was.
///
///   **`ik_target` is the one that retires a ledger entry.** P24.2 shipped IK as
///   a bevy *resource* and wrote down the consequence — "an IK target cannot be
///   authored or saved today" — precisely because v20 was frozen. It is now a
///   component: `inf_ecs::pose::step_pose_evaluation` re-reads it every fixed
///   step, converts each goal out of world space through the entity's own global
///   transform, and solves it before the runtime goals. Because the component
///   rides `EntityRecord` into the `.inf_lvl` bytes a `ScenePayload` already
///   carries, a real `--pie` subprocess engages IK through the door it always
///   used — no test-only injection hook, which is the P21.4 law.
///
///   **`cloth_sim` and `hair_guides` are spent early, deliberately.** A phase
///   gets one scene bump; P24.4 (cloth & hair authoring) would otherwise need a
///   **v22** inside the same phase. Both cost a `None` discriminant byte, and
...
///   record that reaches a class through `EntityRecordGen`'s `V` parameter is
///   re-pointed at it. **No fourth generic was needed** — `V` already exists for
///   exactly this, which is what the v25 rung meant by "the tail is generic", and
///   v24-and-below carry `()` and are untouched.
///
///   What the forty-seven buy: a three-knot torque curve with one shape knob, an
///   eight-slot gearbox with a shift model and reverse as a *gear*, a drivetrain
///   split with per-axle diff locks, brake bias, a simplified-Pacejka tyre (per
///   axis: peak slip, rise stiffness; shared: a sliding plateau and load
///   sensitivity), wheel inertia, centre-of-gravity height, anti-roll bars,
///   downforce with a centre of pressure, sideways drag, a steering rack (rate,
///   return, Ackermann), three driver aids, and the two seat-warp clip times.
///
///   **Why all forty-seven at once.** The house law is one scene bump per phase,
///   and the tuning surface was *known in full* before the first line was
///   written — a field that arrives a wave late costs a whole rung of this
///   ladder for one number. The v26 rung's own note ("some of these fields have
///   no consumer until VIS1b") applies here too and is met the same way: every
///   one of the forty-seven is applied by both hosts and asserted by a store/load
///   arm from the day it lands.
///
...
pub struct EntityRecordGen<T = Terrain, V = Option<VehicleClass>, M = Material> {
    ...
    /// pre-fractures it with, the material strength and density P22.3's
    /// structural solve and chunk bodies read, and the runtime gate. References
    /// no asset — the chunk set is derived from this entity's own `MeshRef` —
    /// so the slot's whole cost to an indestructible level is its `None`
    /// discriminant.
    #[serde(default)]
    pub destructible: Option<Destructible>,
    // ── v21 (P24.3) character components ─────────────────────────
    /// The authored IK goals on this character. Read every fixed step by
    /// `inf_ecs::pose::step_pose_evaluation`, which converts them out of world
    /// space into the character's own frame — so a saved foot plant behaves
    /// identically in Simulate and in the shipped player, through the door both
    /// already used. References no asset (a chain is joint indices into the
    /// skeleton the entity's `SkeletalMesh` already names), so the slot adds no
    /// edge to the cook's dependency closure.
    #[serde(default)]
    pub ik_target: Option<IkTarget>,
    /// A simulated garment: the `.inf_cloth` reference plus the two knobs that
    /// differ per wearer. Authored in v21 and read by P24.4 — see the
    /// [`SCHEMA_VERSION`] ladder for why the slot is spent now.
    #[serde(default)]
    pub cloth_sim: Option<ClothSim>,
    /// Strand hair: the `.inf_hair` reference plus the same two knobs. Same story
    /// as `cloth_sim`.
    #[serde(default)]
...
impl EntityRecordV26 {
    /// Lift to the live record: the forty-seven VEH2a tunables arrive at the
...
impl EntityRecordV25 {
    ...
    /// is what every pre-v26 level meant -- the authored colour, unscaled; and
    /// the VEH2a tunables arrive at their defaults through the v26 rung.
    pub fn into_current(self) -> EntityRecord {
        self.map_material(MaterialV25::into_current).into_current()
    }
...
/// MIRROR: `inf_scene::VehicleClassV26`, which carries the argument in full.
///
...
impl VehicleClassV26 {
    ...
    ///
    /// Two of the fifteen changed what they *mean* without changing their units
    /// or their magnitude, and the lift is honest about both:
    ///
    /// * `max_engine_force_n` was the peak of a flat drive curve and is now the
    ///   driveline **ceiling** — still newtons, still a bound on what the wheels
    ///   are handed, so a v26 car keeps the launch its author gave it;
    /// * `max_speed_mps` was where that curve fell to zero and is now the class's
    ///   **reference** top speed (the steering limit's and the drive camera's),
    ///   with the achieved top speed emerging from the torque curve against the
    ///   drag. A v26 car therefore accelerates past its old ceiling; there is no
    ///   number that could have preserved a wall that no longer exists, and
    ///   pretending otherwise by inventing a torque curve from one force would be
    ///   the codec deciding a tuning question.
...
impl Default for VehicleClassV26 {
    fn default() -> Self {
...
impl EntityRecordV18 {
    ...
    /// Every slot carries through unchanged; the one new slot lifts to `None` —
    /// a level whose ground is a heightfield and nothing else, which is exactly
    /// what a v18 level was.
    ///
    /// v20 inserted [`EntityRecordV19`] between this record and
    /// [`EntityRecord`], and the rung is what keeps a v18 payload loading
    /// forever — the same shape v19 gave the v17 rung.
    pub fn into_v19(self) -> EntityRecordV19 {
        EntityRecordV19 {
            guid: self.guid,
            name: self.name,
            parent: self.parent,
...
/// the `destructible` slot). Identical to the live [`EntityRecord`] except that
...
impl EntityRecordV19 {
    ...
    /// [`EntityRecordV20`] between this record and [`EntityRecord`], and a lift
    ...
    pub fn into_v20(self) -> EntityRecordV20 {
        EntityRecordV20 {
            guid: self.guid,
            name: self.name,
            parent: self.parent,
    ...
    /// Project a live [`EntityRecord`] back onto the frozen v19 shape (the
    /// **downgrade-bless** path that regenerates the committed v19 fixture).
    /// Only the destructible is lost — asserted as a property, not as a field
    /// list, by `v19_entity_downgrade_is_lossless_except_for_the_destructible`.
    ///
    /// Takes the **live** record rather than the frozen one a rung up, so the
    /// bless path always starts from today's truth.
    pub fn from_current(r: EntityRecord) -> Self {
...
impl SceneFileV18 {
    /// Lift every record one rung, to the frozen [`SceneFileV19`] shape. Stamped
    /// v19, not [`SCHEMA_VERSION`] — this is a *rung*, and the version it claims
    /// must match the records it actually holds; `into_current` on the next rung
    /// is what stamps the current schema.
    fn into_v19(self) -> SceneFileV19 {
        SceneFileV19 {
            schema_version: 19,
            title: self.title,
            entities: self
...
/// A frozen schema-v19 file layout, holding [`EntityRecordV19`]s. v20 did not
/// touch [`LevelSettings`], so only `entities` is repointed.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SceneFileV19 {
    pub schema_version: u32,
    pub title: String,
...
impl SceneFileV19 {
    /// Lift every record one rung, to the frozen [`SceneFileV20`] shape. Stamped
    /// **20**, not the current version: the record shapes are v20's, and a file
    /// carrying a v21 stamp over v20 bytes is the exact confusion the frozen
    /// ladder exists to prevent.
    fn into_v20(self) -> SceneFileV20 {
        SceneFileV20 {
            schema_version: 20,
            title: self.title,
            entities: self
...
/// A frozen schema-v20 file layout, holding [`EntityRecordV20`]s. v21 did not
/// touch [`LevelSettings`], so only `entities` is repointed.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SceneFileV20 {
    pub schema_version: u32,
    pub title: String,
...
/// the three character slots). Identical to the live [`EntityRecord`] except that
/// it has **no** `ik_target` / `cloth_sim` / `hair_guides` fields — that is
...
impl EntityRecordV20 {
    ...
    /// with no IK, no garments and no hair, which is exactly what a v20 level
    /// was.
    pub fn into_current(self) -> EntityRecord {
        EntityRecord {
            guid: self.guid,
    ...
    /// Project a live [`EntityRecord`] back onto the frozen v20 shape (the
    ...
    /// field list, by
    /// `v20_entity_downgrade_is_lossless_except_for_the_character_slots`.
    ///
    /// Takes the **live** record rather than the frozen one a rung up, so the
    /// bless path always starts from today's truth.
    pub fn from_current(r: EntityRecord) -> Self {
...
/// MIRROR: `inf_scene::SceneFileV26`.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct SceneFileV26 {
    pub schema_version: u32,
    pub title: String,
    pub entities: Vec<EntityRecordV26>,
    #[serde(default)]
    pub settings: LevelSettings,
    #[serde(default)]
    pub geo: inf_math::geo::GeoAnchor,
    ...
    fn into_current(self) -> SceneFile {
        SceneFile {
            ...
            entities: self
                .entities
                .into_iter()
                .map(EntityRecordV26::into_current)
                .collect(),
            settings: self.settings,
            geo: self.geo,
        }
...
pub fn record_of(doc: &SceneDoc, guid: Uuid) -> Option<EntityRecord> {
    ...
    Some(EntityRecord {
        ...
        destructible: w.get::<Destructible>(e).copied(),
        // v21 (P24.3). `IkTarget` is CLONED rather than copied: its goal list is
        // a `Vec`, which is why it is the first non-`Copy` character component.
...
pub fn decode(bytes: &[u8]) -> Result<SceneFile, String> {
    ...
    match header.schema_version {
        // **`1`, not `0 | 1`** (L5.F4). `SceneFileV1` is
        // `{ schema_version: u32, title: String, entities: Vec<_> }`, and under
        // `bincode::config::standard()` those are three varints — so three zero
        // bytes decode as `{0, "", []}`, `decode_from_slice` ignores the tail,
        // and **any all-zero buffer of at least three bytes used to load as a
        // valid, untitled, empty level.** That is exactly what a preallocated,
        // sparse or filesystem-recovered partial write leaves behind: "your
        // level is now empty" beats no message, but "this file is not a level"
        // is the correct answer.
        //
        // The `0` arm bought nothing to lose: `encode` has always written
        // `SCHEMA_VERSION`, so no `.inf_lvl` in existence has version 0.
        //
        // **Not** the recorded `AssetPayload::migrate` bound
        // (`docs/ROADMAP.md`, pinned by `inf_asset::payload`'s own test): that
        // one is scoped to a different codec, has no cheap fix, and is
        // ledgered. This one is a different decoder with no entry, no pin, and
        // a free repair. Wire bytes unchanged — this is a version the writer
        // never emitted.
        1 => {
        ...
        // Version 0 is not a level from a newer build; it is not a level.
        0 => Err(
            "this file is not a level: its schema version is 0, which no build has ever written \
             (a zero-filled or truncated file reads this way)"
                .to_string(),
        ),
        n => Err(format!(
...
pub fn migrate(file: SceneFile) -> Result<SceneFile, String> {
    ...
    // (v1→…→v20→v21); nothing more to do here. Future upgrades chain in `decode`.
    Ok(file)
}
...
pub(crate) fn write_record_components(
    ...
    copy_slot!(&rec.destructible, Destructible);
    // v21 (P24.3) — the IK goals clone (a `Vec` inside), the other two copy.
...
mod tests {
    ...
    /// The **v6 rung** of the reference scene (L5.F6), derived from
    /// [`v7_reference`] through the frozen v6 record.
    ///
    /// v6 → v7 changed exactly one thing: `MeshRef` gained its `asset` field, so
    /// the v6 record carries [`MeshRefV6`] (primitive only) — a lossless
    /// downgrade of this scene, whose meshes are all primitives.
    fn v6_reference() -> SceneFileV6 {
        let v7 = v7_reference();
        SceneFileV6 {
            schema_version: 6,
            // Its own title, so a fixture in the wrong slot says so rather than
            // passing as its neighbour.
    ...
    /// Write the committed v6 fixture (L5.F6) under `INF_BLESS_FIXTURES=1`.
    #[test]
    fn bless_v6_fixture() {
        if std::env::var("INF_BLESS_FIXTURES").is_err() {
            return;
        }
        let bytes = bincode::serde::encode_to_vec(v6_reference(), bincode_config()).unwrap();
        let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("tests/fixtures/scene_v6.inf_lvl");
        std::fs::write(&path, &bytes).expect("write v6 fixture");
        eprintln!("blessed v6 fixture: {}", path.display());
    }

    /// **v6 was the one rung of the ladder with no committed fixture on either
    /// mirror** (L5.F6) — v5 below it and v7 above it are both byte-pinned, and
    /// v6 is the rung that appended the joint and audio slots. A silent edit to
    /// this mirror's v6 records, or a drift from `inf-scene`'s, failed no test.
    #[test]
    fn v6_fixture_loads_forever_and_lifts() {
        let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("tests/fixtures/scene_v6.inf_lvl");
        if !path.exists() {
            eprintln!("SKIP: v6 fixture not blessed yet ({})", path.display());
            return;
        }
        let bytes = std::fs::read(&path).unwrap();
        assert_eq!(bytes[0], 6, "fixture must be a genuine schema-v6 payload");
        let rebuilt = bincode::serde::encode_to_vec(v6_reference(), bincode_config()).unwrap();
        assert_eq!(
            rebuilt, bytes,
            "the committed v6 fixture must match our frozen v6 writer"
        );

        let file = decode(&bytes).expect("v6 fixture decodes");
        assert_eq!(file.schema_version, SCHEMA_VERSION);
        assert_eq!(file.title, "V6 Fixture Level");
        let by_name = |n: &str| file.entities.iter().find(|r| r.name == n).unwrap();
        // The slots v6 EXISTS for are carried, not defaulted away.
        assert!(
            by_name("Cone").joint_3d.is_some(),
            "v6 is the rung that appended the joint slots; the fixture must exercise one"
        );
        // The v7 field v6 does not have lifts to its default.
        assert_eq!(by_name("Ground").mesh.unwrap().asset, None);
        assert_eq!(by_name("Ground").mesh.unwrap().primitive, Primitive::Plane);
        assert_eq!(file.settings.sim_hz, 120.0);
    }

    /// **The cross-read arm** (L5.F6): this mirror decodes `inf-scene`'s
    /// committed v6 fixture.
    ///
    /// v10–v21 are already cross-pinned this way; v6 was not, so the two v6
    /// rungs could drift apart with nothing to notice. The two fixtures are
    /// deliberately *different scenes* (each crate's own reference), so this
    /// asserts the two CODECS agree — not that the two files are the same file.
    #[test]
    fn the_ring0_v6_fixture_decodes_here_too() {
        let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../../../crates/inf-scene/tests/fixtures/scene_v6.inf_lvl");
        if !path.exists() {
            eprintln!(
                "SKIP: inf-scene v6 fixture not present ({})",
                path.display()
            );
            return;
        }
        let bytes = std::fs::read(&path).unwrap();
        assert_eq!(bytes[0], 6);
        let file = decode(&bytes).expect("inf-scene's v6 fixture decodes in the editor mirror");
        assert_eq!(file.schema_version, SCHEMA_VERSION);
        assert_eq!(file.title, "V6 Fixture Level");
        assert!(
            file.entities.iter().any(|r| r.joint_3d.is_some()),
            "the v6 joint slot survived the cross-mirror read"
        );
    }
    ...
    fn the_frozen_tile_generation_covers_this_schema() {
        assert_eq!(
            ...
            "the scene schema moved. Generation-1 frozen tiles (TerrainTileFrozenV1, via \
             ...
             generation table. (v17..v20 are all the latter case: each appended an entity \
             slot and left every tile layout alone — v19's voxel volume extends the ground \
    ...
    fn v18_entity_downgrade_is_lossless_except_for_the_voxel_volume() {
        ...
        let back = EntityRecordV18::from_current(live.clone())
            .into_v19()
            .into_v20()
            .into_current();
    ...
    fn v19_costs_one_byte_per_voxel_free_entity() {
        let live = v9_base(uuid::Uuid::from_u128(0xF900), "Bedrock", None)
            ...
            .into_v13()
            .into_current();
        // Measured between the two FROZEN rungs, not against the live record:
        // v20 appended a slot of its own, and comparing today's record with the
        // v18 one would silently report the two bumps' combined price as v19's.
        let v19 = bincode::serde::encode_to_vec(
            EntityRecordV19::from_current(live.clone()),
            bincode_config(),
        )
        .unwrap();
        let v18 = bincode::serde::encode_to_vec(
        ...
        let carved = EntityRecordV19 {
            voxel_volume: Some(VoxelVolume::default()),
            ..EntityRecordV19::from_current(live)
        };
        assert!(
    ...
    // ── schema v20 (P22.2 destruction) ────────────────────────────────────

    /// An all-`None` frozen v19 entity — the struct-update base for
    /// [`v19_reference`]. Built through the downgrade hop so the field list can
    /// never drift from the live record.
    fn v19_base(guid: uuid::Uuid, name: &str, parent: Option<uuid::Uuid>) -> EntityRecordV19 {
        EntityRecordV19::from_current(
            v9_base(guid, name, parent)
                .into_v10()
                .into_v11()
                .into_v12()
    ...
    /// defaults.
    ///
    /// The literals must match `inf-scene`'s `v19_fixture_volume` exactly — the
    /// two committed fixtures are byte-compared by
    /// [`v19_fixture_matches_the_runtime_codecs_copy`], which is the whole point
    /// of writing them twice.
    ...
    /// the v16 fixture's terrain, the v17 fixture's river and the v18 fixture's
        ...
        SceneFileV19 {
            ...
            entities: vec![
                ...
                EntityRecordV19 {
                    ...
                    water_body: Some(v17_fixture_water()),
                    ..v19_base(g(0xF204), "River", None)
                },
                EntityRecordV19 {
                    rigid_body_3d: Some(RigidBody3D::default()),
                    buoyancy: Some(v18_fixture_buoyancy()),
                    ..v19_base(g(0xF205), "Raft", None)
                },
    ...
    /// Write the committed v19 fixture from [`v19_reference`] under
    /// `INF_BLESS_FIXTURES=1` (the temporary-writer discipline). Never hand-edit
    /// the committed bytes.
    #[test]
    fn bless_v19_fixture() {
        if std::env::var("INF_BLESS_FIXTURES").is_err() {
            return;
        }
        let bytes = bincode::serde::encode_to_vec(v19_reference(), bincode_config()).unwrap();
        assert_eq!(bytes[0], 19);
        let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("tests/fixtures/scene_v19.inf_lvl");
        std::fs::write(&path, &bytes).expect("write v19 fixture");
        eprintln!(
            "blessed v19 fixture: {} ({} bytes)",
            path.display(),
            bytes.len()
        );
    }
    ...
    fn v19_fixture_is_reproducible_and_genuinely_v19() {
        ...
        let rebuilt = bincode::serde::encode_to_vec(v19_reference(), bincode_config()).unwrap();
        assert_eq!(
            rebuilt, bytes,
    ...
    /// This crate's committed v19 fixture must be **byte-identical** to the Ring-0
    /// runtime reader's — the two codecs are one wire contract written twice.
    #[test]
    fn v19_fixture_matches_the_runtime_codecs_copy() {
        let mine = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("tests/fixtures/scene_v19.inf_lvl");
        let theirs = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../../../crates/inf-scene/tests/fixtures/scene_v19.inf_lvl");
        assert_eq!(
            std::fs::read(&mine).expect("editor v19 fixture"),
            std::fs::read(&theirs).expect("runtime v19 fixture"),
            "the two v20-bump fixtures diverged — the codecs are no longer mirrors"
        );
    }

    /// The committed v19 fixture — written by the **pre-v20 codec**, before the
    /// entity record grew its `destructible` slot — still loads, with the v19
    /// content (the cavern's volume included) preserved verbatim and nothing made
    /// breakable. The "old bytes load forever" gate for the v20 bump.
    #[test]
    fn v19_loads_and_lifts_without_a_destructible() {
        let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("tests/fixtures/scene_v19.inf_lvl");
        let file = decode(&std::fs::read(&path).unwrap()).expect("v19 fixture decodes");
        assert_eq!(file.schema_version, SCHEMA_VERSION);
        assert_eq!(file.title, "V19 Fixture Level");
        assert_eq!(file.entities.len(), 6);
        let by_name = |n: &str| file.entities.iter().find(|r| r.name == n).unwrap();

        // The v19 content survives the frozen-record hop intact — including the
        // slot v19 itself added, which is the half a defaults-only fixture would
        // never have proven …
        assert_eq!(
            by_name("Cube").mesh.unwrap().asset,
            Some(uuid::Uuid::from_u128(0xF2A1))
        ...
        // … and the one new slot lifts to `None` — a level in which nothing
        // breaks, which is exactly what a v19 level was.
        for r in &file.entities {
            assert!(
                r.destructible.is_none(),
                "a v19 level has nothing destructible; the lift must not conjure any"
            );
        }
    }

    /// The v19 downgrade is lossless **except** for the destructible — the one
    /// thing v19 cannot express. Proven as a property (round-trip a live record
    /// through the frozen shape) rather than by listing fields, so a slot added
    /// later cannot silently fall out of the ladder.
    #[test]
    fn v19_entity_downgrade_is_lossless_except_for_the_destructible() {
        let live = EntityRecord {
            terrain: Some(v16_fixture_terrain()),
            spline: Some(Spline::default()),
            water_body: Some(v17_fixture_water()),
            ...
            ..v9_base(uuid::Uuid::from_u128(0xFA00), "Wall", None)
                .into_v10()
                .into_v11()
                .into_v12()
                .into_v13()
        ...
        let back = EntityRecordV19::from_current(live.clone())
            .into_v20()
            .into_current();

        // The destructible is exactly what is lost — the voxel volume and the
        // buoyancy, which v19 *can* express, are not …
        assert!(back.destructible.is_none());
        ...
        assert_eq!(
            EntityRecord {
                destructible: live.destructible,
                ..back
            },
            live,
            "the v19 downgrade lost something other than the destructible"
        );
    }

    /// The non-default `Destructible` the v20 tests author: every field away
    ...
    /// The v20 addition round-trips through the whole editor codec — including
    /// the **new decode arm**, which only a payload stamped v20 exercises — and a
    /// `Destructible` really does reach an ECS world through
    /// `write_record_components` (the read half of the codec, which a bytes-only
    /// test would never touch).
    #[test]
    fn v20_destructible_round_trips_through_the_codec_and_the_world() {
        let d = v20_fixture_destructible();
        let file = SceneFile {
            schema_version: SCHEMA_VERSION,
            title: "Rubble".into(),
            entities: vec![EntityRecord {
                destructible: Some(d),
                ..v9_base(uuid::Uuid::from_u128(0xFB00), "Wall", None)
                    .into_v10()
                    .into_v11()
                    .into_v12()
                    .into_v13()
        ...
        assert_eq!(bytes[0], SCHEMA_VERSION as u8);
        let back = decode(&bytes).expect("the current schema decodes");
        assert_eq!(back.entities[0].destructible, Some(d));
        // Re-encoding is byte-identical.
        assert_eq!(
            bincode::serde::encode_to_vec(&back, bincode_config()).unwrap(),
            bytes
        ...
        let mut doc = SceneDoc::new();
        apply_to_doc(&mut doc, &back);
        let guid = uuid::Uuid::from_u128(0xFB00);
        let e = doc.world().entity_of(guid).unwrap();
        assert_eq!(doc.world().world().get::<Destructible>(e).copied(), Some(d));
        assert_eq!(record_of(&doc, guid).unwrap().destructible, Some(d));
    }

    /// **The v20 price, isolated**: an entity that cannot break pays exactly one
    /// discriminant byte.
    ///
    /// Measured between the two **FROZEN** rungs, never against the live record
    /// — the P22.2 lesson, met again the moment v21 landed: this used to compare
    /// today's encoding with the v19 one, and the day v21 appended three slots it
    /// reported both bumps' combined price as v20's.
    #[test]
    fn v20_costs_one_byte_per_indestructible_entity() {
        let live = v9_base(uuid::Uuid::from_u128(0xFC00), "Bedrock", None)
            .into_v10()
            .into_v11()
            .into_v12()
            .into_v13()
            .into_current();
        let v20 = bincode::serde::encode_to_vec(
            EntityRecordV20::from_current(live.clone()),
            bincode_config(),
        )
        .unwrap();
        let v19 = bincode::serde::encode_to_vec(
        ...
        // A record that *carries* a destructible costs more than its own
        // discriminant — and never more than its five fields, because the chunks
        // live in the derived `.inf_fracture`, not in the level.
        let breakable = EntityRecordV20::from_current(EntityRecord {
            destructible: Some(Destructible::default()),
            ..live
        });
        // Against the FROZEN rung on both sides, so this stays a statement about
        // v20 and does not quietly become one about however many slots the live
        // record has grown since.
        assert!(
            bincode::serde::encode_to_vec(&breakable, bincode_config())
                .unwrap()
                .len()
                > v20.len() + 1
        );
    ...
    // ── schema v21 (P24.3 character components) ───────────────────────────
    ...
    ///
    /// Built by lifting [`v19_reference`] one rung and then authoring the slot
    /// v20 added, so the field list can never drift from the ladder.
    fn v20_reference() -> SceneFileV20 {
        let v19 = v19_reference();
    ...
    /// Write the committed v20 fixture from [`v20_reference`] under
    /// `INF_BLESS_FIXTURES=1` (the temporary-writer discipline). Never hand-edit
    /// the committed bytes.
    #[test]
    fn bless_v20_fixture() {
        if std::env::var("INF_BLESS_FIXTURES").is_err() {
            return;
        }
        let bytes = bincode::serde::encode_to_vec(v20_reference(), bincode_config()).unwrap();
        assert_eq!(bytes[0], 20);
        let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("tests/fixtures/scene_v20.inf_lvl");
        std::fs::write(&path, &bytes).expect("write v20 fixture");
        eprintln!(
            "blessed v20 fixture: {} ({} bytes)",
            path.display(),
            bytes.len()
        );
    }
    ...
    fn v20_fixture_is_reproducible_and_genuinely_v20() {
        ...
        let rebuilt = bincode::serde::encode_to_vec(v20_reference(), bincode_config()).unwrap();
        assert_eq!(
            rebuilt, bytes,
    ...
    /// This crate's committed v20 fixture must be **byte-identical** to the Ring-0
    /// runtime reader's — the two codecs are one wire contract written twice.
    #[test]
    fn v20_fixture_matches_the_runtime_codecs_copy() {
        let mine = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("tests/fixtures/scene_v20.inf_lvl");
        let theirs = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../../../crates/inf-scene/tests/fixtures/scene_v20.inf_lvl");
        assert_eq!(
            std::fs::read(&mine).expect("editor v20 fixture"),
            std::fs::read(&theirs).expect("runtime v20 fixture"),
            "the two v21-bump fixtures diverged — the codecs are no longer mirrors"
        );
    }

    /// The committed v20 fixture — written by the **pre-v21 codec** — still
    /// loads, with the v20 content preserved verbatim and no character
    /// components conjured. The "old bytes load forever" gate for the v21 bump.
    #[test]
    fn v20_loads_and_lifts_without_the_character_slots() {
        let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("tests/fixtures/scene_v20.inf_lvl");
        let file = decode(&std::fs::read(&path).unwrap()).expect("v20 fixture decodes");
        assert_eq!(file.schema_version, SCHEMA_VERSION);
        assert_eq!(file.title, "V20 Fixture Level");
        assert_eq!(file.entities.len(), 6);
        let by_name = |n: &str| file.entities.iter().find(|r| r.name == n).unwrap();

        // Everything the earlier rungs authored survives TWO hops …
        assert_eq!(by_name("Sun").light.unwrap().intensity, 2.0);
        assert_eq!(file.settings.sim_hz, 90.0);
        assert_eq!(by_name("River").water_body, Some(v17_fixture_water()));
        assert_eq!(by_name("Raft").buoyancy, Some(v18_fixture_buoyancy()));
        ...
        // … and the three new slots lift to `None`.
        for r in &file.entities {
            assert!(
                r.ik_target.is_none() && r.cloth_sim.is_none() && r.hair_guides.is_none(),
    ...
    /// Proven as a property (round-trip a live record through the frozen shape)
    /// rather than by listing fields, so a slot added later cannot silently fall
    /// out of the ladder.
    #[test]
    fn v20_entity_downgrade_is_lossless_except_for_the_character_slots() {
        let live = EntityRecord {
            ...
            ..v9_base(uuid::Uuid::from_u128(0xFA10), "Hero", None)
                .into_v10()
                .into_v11()
                .into_v12()
                .into_v13()
        ...
        let back = EntityRecordV20::from_current(live.clone()).into_current();
    ...
    /// from its default, so a hop that produced defaults would be caught.
    ///
    /// The literals must match the runtime codec's `v21_fixture_ik_target`.
    fn v21_fixture_ik_target() -> IkTarget {
        ...
        IkTarget {
            goals: vec![
                IkGoalRecord {
                    chain: vec![3, 4, 5],
                    target_entity: inf_ecs::refs::EntityRef::new(uuid::Uuid::from_u128(0xF_1CE0)),
    ...
    fn v21_fixture_cloth() -> ClothSim {
        ClothSim {
            asset: Some(uuid::Uuid::from_u128(0xF_C107)),
            enabled: false,
            quality: 3,
        }
    }
    ...
    fn v21_fixture_hair() -> HairGuides {
        HairGuides {
            asset: Some(uuid::Uuid::from_u128(0xF_4A12)),
            enabled: false,
            quality: 2,
        }
    }

    /// The v21 additions round-trip through the whole editor codec — including
    /// the **new decode arm**, which only a payload stamped v21 exercises — and
    /// they really do reach an ECS world through `write_record_components` (the
    /// read half of the codec, which a bytes-only test would never touch).
    #[test]
    fn v21_character_slots_round_trip_through_the_codec_and_the_world() {
        let ik = v21_fixture_ik_target();
        let cloth = v21_fixture_cloth();
        let hair = v21_fixture_hair();
        let file = SceneFile {
            schema_version: SCHEMA_VERSION,
            title: "Rigged".into(),
            entities: vec![EntityRecord {
                ik_target: Some(ik.clone()),
                cloth_sim: Some(cloth),
                hair_guides: Some(hair),
                ..v9_base(uuid::Uuid::from_u128(0xFB10), "Hero", None)
                    .into_v10()
                    .into_v11()
                    .into_v12()
                    .into_v13()
        ...
        assert_eq!(bytes[0], SCHEMA_VERSION as u8);
        let back = decode(&bytes).expect("the current schema decodes");
        assert_eq!(back.entities[0].ik_target.as_ref(), Some(&ik));
        assert_eq!(back.entities[0].cloth_sim, Some(cloth));
        assert_eq!(back.entities[0].hair_guides, Some(hair));
        // Re-encoding is byte-identical.
        assert_eq!(
            bincode::serde::encode_to_vec(&back, bincode_config()).unwrap(),
            bytes
        ...
        // … and they land on a real entity, which is what `record_of` reads back.
        let mut doc = SceneDoc::new();
        apply_to_doc(&mut doc, &back);
        let guid = uuid::Uuid::from_u128(0xFB10);
        let e = doc.world().entity_of(guid).unwrap();
        assert_eq!(doc.world().world().get::<IkTarget>(e).cloned(), Some(ik));
        assert_eq!(doc.world().world().get::<ClothSim>(e).copied(), Some(cloth));
        assert_eq!(
            doc.world().world().get::<HairGuides>(e).copied(),
            Some(hair)
        );
        let rec = record_of(&doc, guid).unwrap();
        assert_eq!(rec.cloth_sim, Some(cloth));
        assert_eq!(rec.hair_guides, Some(hair));
        // The goal list survives the world round trip field-for-field, the
        // disabled goal included — a hop that dropped `enabled: false` would
        // still round-trip the vector's length.
        let goals = rec.ik_target.unwrap().goals;
        assert_eq!(goals.len(), 2);
        assert_eq!(goals[0].weight, 0.75);
        assert_eq!(
            goals[0].target_entity.get(),
            Some(uuid::Uuid::from_u128(0xF_1CE0))
        );
    ...
    /// `SceneFile`, which is true of any struct whatsoever, and the other
    ...
    fn the_v27_wire_shape_is_pinned_against_an_independent_declaration() {
        ...
        let level = SceneFile {
            ...
            entities: vec![
                ...
                v9_base(uuid::Uuid::from_u128(0xFD11), "Prop", None)
                    .into_v10()
                    .into_v11()
                    .into_v12()
                    .into_v13()
        ...
        //
        // Ring 1 did not seed a class here until wave VEH2a, so this half of the
        // mirror was pinning `None` and could not have failed. It is seeded now,
        // which is what makes the Ring-1 pin the equal of `inf_scene`'s twin.
    ...
    fn a_tail_slot_appended_without_a_bump_leaves_bytes_the_pin_refuses() {
        ...
        let one_entity = bincode::serde::encode_to_vec(
            &SceneFile {
    ...
    /// `inf_scene`'s `fixture_vehicle_class`, and byte-identical to it, because the
    ...
    fn v26_downgrade_is_lossless_except_for_what_v27_added() {
        ...
        // Built through `record_of` rather than by literal, so this arm also
        // fails if the projection out of the world stops carrying the slot.
        ...
        let rung = EntityRecordV26::from_current(live.clone()).into_current();
    ...
    /// refusal — the two codecs are read side by side and must agree about what
    /// is not a level. See `inf_scene`'s twin for the mechanism.
    #[test]
    fn a_zero_filled_buffer_is_not_a_level() {
        for len in [3usize, 8, 64, 4096] {
            ...
            assert!(
                err.contains("version is 0"),
                "the refusal must name the cause, not blame a newer editor: {err}"
            );
        }
