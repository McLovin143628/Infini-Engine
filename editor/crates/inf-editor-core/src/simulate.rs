...
pub struct SimSession {
    ...
    /// [`enter`](Self::enter) and **grown by `engine.spawn`** (SCRIPT3).
    entities: BTreeMap<i64, Uuid>,
    /// Guids the running handler destroyed (SCRIPT3), drained into
    /// [`actors`](Self::actors) at the end of [`run_on_guid`](Self::run_on_guid).
    despawned: Vec<Uuid>,
    ...
    /// `RuntimeSim::holds`. Advanced with the *fixed* `dt`, so a press duration
    /// is a function of the simulation and the preview cannot classify a press
    /// differently from the shipped player.
...
impl SimSession {
    ...
    fn apply_pending_tunes(&mut self, doc: &mut SceneDoc) {
        ...
        for (tune, scope) in std::mem::take(&mut self.pending_tunes) {
            ...
            let applied = match &tune {
                ...
                //    `inf_ecs::item`'s header gives. Same shape as the two
                //    above: session-scoped, and it answers with a value.
                crate::tuning::Tune::Weapon {
                    item_id,
                    name,
                    value,
                } => {
                    let defs = inf_ecs::item::item_defs_mut(doc.world_mut());
                    match defs.0.get_mut(&inf_ecs::item::canonical_id(item_id)) {
                        Some(def) => match def.weapon.as_mut() {
                            Some(w) => w.set(name, *value),
                            None => false,
                        },
                        None => false,
                    }
    ...
    /// Set it, guarded (I5). Non-finite falls back to the default; finite
    /// clamps into `inf_ecs::movement::PRESS_THRESHOLD_RANGE_S`. MIRROR of
    /// `RuntimeSim::set_press_threshold_s`, and the guard is restated rather
    /// than shared because a value can arrive at either host without passing
    /// through the other's settings file.
    ...
    fn fixed_step(&mut self, doc: &mut SceneDoc) {
        ...
        self.drain_dispatch(doc); // Wave 3: Tick may dispatch custom events.
                                  ...
                                  //    this step's own held set — see `RuntimeSim::fixed_step`, of which
                                  //    this is the MIRROR, for why a frame time would make the preview and
                                  //    the shipped player classify the same press differently.
        ...
        //    (MIRROR of `RuntimeSim::fixed_step`.)
        let mut report = inf_physics::d3::step_gameplay(doc.world_mut(), &mut self.bridge3d, dt);
    ...
    fn run_on_guid(
        ...
        // SCRIPT3: `engine.destroy` took an entity out of the world; if it was an
        // actor its handlers stop with it. Drained AFTER the re-insert above, so
        // an actor that destroyed itself does not come back.
    ...
    fn fire_weapon_audio(&mut self, world: &EcsWorld, hits: &[inf_physics::d3::WeaponHit]) {
        ...
        for hit in hits {
            ...
            let Some((src, _)) = destroyed_emitter(world, target) else {
                continue;
            };
            let cmd =
...
struct SimHost<'a> {
    ...
    /// The blueprint `i64 → Guid` map, **mutable since SCRIPT3**: `engine.spawn`
    /// adds a row and `engine.destroy` takes one away, so a script-spawned
    /// entity is addressed through the one map an actor is, rather than through
    /// a second scheme beside it. (`inf_mod::tick` has taken it mutably since
    /// P14.5 for the same reason — a spawning mod.)
    entities: &'a mut BTreeMap<i64, Uuid>,
    /// Guids `engine.destroy` removed from the world during this handler, drained
    /// by the session after it finishes: if one of them was an ACTOR, its
    /// handlers must stop with its entity.
